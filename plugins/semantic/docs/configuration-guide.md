...
| 阈值 | 效果 | 适用场景 |
|------|------|---------|
| `0.5` | 较宽松 | 返回更多结果但可能包含不相关内容 |
| `0.6` | 宽松 | 初次探索、广泛搜索 |
| `0.7` | **推荐** | 日常使用，平衡准确度和召回率 |
| `0.8` | 较严格 | 精确查找，只返回高度相关的结果 |
| `0.9+` | 非常严格 | 可能错过部分相关结果 |
...
相似度: 0.68 - UserSession.validate()         ✗ (低于 0.7)
相似度: 0.55 - UserProfile.update()           ✗ (低于 0.7)
相似度: 0.42 - UserSettings.get()             ✗ (低于 0.7)
```
