...
def test_command_merely_mentions_skein_stays_silent() -> None:
    """搜索文本含 skein 不等于调用 skein 命令。"""
    assert not _report("grep skein README.md", "no matches")


def test_empty_error_stays_silent() -> None:
    """无错误正文时没有可执行指导, 保持静默。"""
    assert not _report("skein status", "")
