...
直接 import 纯函数所在模块 —— 进度算法在 `skeinlib.task.dag`, 状态常量在 `skeinlib.model`,
...
覆盖场景:
  ...
  6. SubtaskStatus.FAILED 与 SubtaskStatus.RUNNING 同区间 (10-90): 有验收插值 / 无验收中点
...
def _sub(status: str = SubtaskStatus.PENDING, crit: int = 0, done: int = 0) -> dict[str, Any]:
...
def test_subs_all_done_active_task_capped_at_85() -> None:
    t = _task(TaskStatus.ACTIVE, [_sub(SubtaskStatus.DONE), _sub(SubtaskStatus.DONE), _sub(SubtaskStatus.DONE)])
...
def test_subs_mostly_done_check_task_floor_interp() -> None:
    subs = [_sub(SubtaskStatus.DONE)] * 13 + [_sub(SubtaskStatus.PENDING)]
    # sub_avg = (13*100 + 2) / 14 = 93.0；check 区间 (85,95) 插值 = 85+10*93/100 = 94.3 → floor 94
    assert _task_pct(_task(TaskStatus.CHECK, subs)) == 94


# 3. subs 按验收部分通过 (pending, 4 验收通过 3) → _sub_pct 区间插值 (非直接百分比)
def test_subs_acceptance_partial_pass_interp() -> None:
    subs = [_sub(SubtaskStatus.PENDING, crit=4, done=3), _sub(SubtaskStatus.PENDING, crit=4, done=3)]
    ...
    assert _task_pct(_task(TaskStatus.ACTIVE, subs)) == 12


# 4. 无 subs 各状态取区间中点 (非区间下限)
def test_no_subs_status_takes_range_midpoint() -> None:
    assert _task_pct(_task(TaskStatus.PENDING)) == 2   # (0+5)//2
    assert _task_pct(_task(TaskStatus.RESEARCH)) == 7  # (5+10)//2
    assert _task_pct(_task(TaskStatus.ACTIVE)) == 47   # (10+85)//2
    assert _task_pct(_task(TaskStatus.CHECK)) == 90    # (85+95)//2
    assert _task_pct(_task(TaskStatus.DONE)) == 100


# 5. 空 subtasks list (而非缺字段) 与无 subs 字段等价 → 同走区间中点分支
def test_empty_subtasks_list_falls_through_to_midpoint() -> None:
    assert _task_pct(_task(TaskStatus.ACTIVE, subs=[])) == 47
    assert _task_pct(_task(TaskStatus.CHECK, subs=[])) == 90


# 6. SubtaskStatus.FAILED 与 SubtaskStatus.RUNNING 同区间 (10,90)：有验收插值 / 无验收中点
def test_failed_subtask_shares_running_range() -> None:
    # 失败 + 验收 2/3 → 10+80*2/3 = 63.33 → floor 63
    assert _sub_pct(_sub(SubtaskStatus.FAILED, crit=3, done=2)) == 63
    assert _sub_pct(_sub(SubtaskStatus.RUNNING, crit=3, done=2)) == 63
    # 失败 + 无验收 → 区间中点 (10+90)//2 = 50
    assert _sub_pct(_sub(SubtaskStatus.FAILED)) == 50
    assert _sub_pct(_sub(SubtaskStatus.RUNNING)) == 50


# 7. floor 取整边界：插值结果带 .5+ 小数时向下取整，非四舍五入
def test_interp_floors_not_rounds_on_half_up_fraction() -> None:
    # running 区间 (10,90)，4/7 插值 = 10+80*4/7 = 55.714... → floor 55 (round 会给 56)
    assert _sub_pct(_sub(SubtaskStatus.RUNNING, crit=7, done=4)) == 55
    # task 层：done(100) + pending 中点(2) 均值 51.0；check 区间 (85,95) 插值 = 85+10*0.51 = 90.1 → floor 90
    subs = [_sub(SubtaskStatus.DONE), _sub(SubtaskStatus.PENDING)]
    assert _task_pct(_task(TaskStatus.CHECK, subs)) == 90
