...
# 子进程覆盖率: 套件大半用例是真跑 CLI 的 subprocess, 不接线的话它们执行到的代码一行都不计入
# (实测总覆盖率被低估近一半)。只在父进程本身跑在 coverage 下时才开, 否则平时跑测试白付启动开销。
if os.environ.get("COVERAGE_RUN") or "coverage" in sys.modules or os.environ.get("COV_CORE_SOURCE"):
    os.environ.setdefault("SKEIN_COVERAGE_DATA_FILE", str(SCRIPTS / ".coverage"))
    os.environ.setdefault("COVERAGE_PROCESS_START", str(SCRIPTS / ".coveragerc"))
