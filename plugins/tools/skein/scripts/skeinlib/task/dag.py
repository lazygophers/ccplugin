...
def _split_semi(s: Optional[str] | list[str]) -> list[str]:
    # 验收 checklist 用分号分隔 (条目内可含逗号); `--check` 可重复传, 各段依次累加
    parts = [s] if isinstance(s, str) else list(s or [])
    return [x.strip() for p in parts for x in p.split(";") if x.strip()]
