...
BIN_RE = re.compile(
    r"(?:^\s*|[;&|(]\s*|\n\s*)(?:(?:python\d*|uv\s+run)\s+)?[\"']?"
    r"(?:[^\s;&|()\"']*/)?(?:skein-spec|skein|skein\.py|spec\.py)[\"']?(?:\s|$)"
)
...
def cmd_report(payload: dict[str, Any]) -> int:
    ...
    if not error or not BIN_RE.search(command):
        return 0
    output: dict[str, Any] = {}
...
__all__ = ["BIN_RE", "ISSUE_URL", "TRACEBACK_MARK", "_TRACEBACK_MARK", "cmd_report"]
