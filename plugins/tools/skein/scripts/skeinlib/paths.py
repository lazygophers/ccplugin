...
# ── 路径基准 (单一真值源) ──────────────────────────────────────────────────────
# 拆包时踩过两次: 代码从 scripts/skein.py 搬进 scripts/skeinlib/*.py, 所有 `Path(__file__).parent`
# 静默深了一层 —— 表现是 `setup` 找不到 spec.py、serve 找不到前端资产, 而单测全绿 (那些路径只在
# 真跑命令时才解析)。所以基准只在这里算一次, 别处一律引用这三个常量, 禁再写 `__file__` 推路径。
