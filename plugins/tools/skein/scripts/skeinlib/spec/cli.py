...
def main() -> None:
    ...
    an.add_argument("tid", help="task id (.skein/task/<tid>/ 下须有 task.json, design.md 可选)")
    ...
    fc = sub.add_parser("finish-candidates", help="[finish 用] 为 task 生成候选 product wiki 页 (三路降级: anchors反查→TaskSpec关键词recall→皆无建议新建)")
    fc.add_argument("tid", help="task id (.skein/task/<tid>/ 下须有 task.json)")
