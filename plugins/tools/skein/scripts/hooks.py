...
- **inline**：纯查询/问答/单文件单处
- **倾向**x: 现有补充 -> flow -> inline
- 如果无法判断，通过 AskUserQuestion 和用户确认，AI 应该倾向走任务
- 如果确认走任务，应该通过 Skills(skein-flow) 走完整的任务流程
- **注意**：不要因为用户输入直接中断前面正在进行的，要确保前置的内容不会因此中断、丢失
- 注意任务拆分要合理，用户提出的可能是一个task，也可能是两个task，也可能是一部分是已有的task，剩余的是两个task，要注意甄别
"""
...
def cmd_user_prompt(d: dict[str, Any]) -> int:
    ...
    if prompt.startswith("/skein-") or prompt.startswith("/skein:skein-") or prompt == "go" or prompt == "exec" or prompt == "do" or prompt.startswith("skein-"):
        return 0
    root = _git_root(d.get("cwd") or os.getcwd())
    ...
    if not os.path.exists(os.path.join(dir_, "config.yaml")):
        ...
        ac_txt = "启用 (finish/阶段切换自动 commit)" if ac else "禁用 (改动需手动 commit)"
        ctx += f"\n\n# SKEIN 运行配置\n- worktree: {wt_txt}\n- 最大并行 subtask: {ma}\n- auto_commit: {ac_txt}"
    print(json.dumps({"hookSpecificOutput": {
