...
3. **有 `✗`** → 逐条向用户说明违规项 + 建议修法 (给出具体命令供按检出项填充: subtask 态/验收错 → `skein subtask fail <tid> <sid> --note "原因"` / `skein subtask check <tid> <sid> --passed all` / `skein subtask done <tid> <sid>`; 顶层索引与 per-task 不一致 → `skein task status <tid>` 核对后重跑对应生命周期命令 `skein task confirm|finish <id>` 重建), **禁自动改** `.skein/` (违规多为流程错误, 需用户确认)。
4. **仅 `⚠`** → 列出提醒, 不阻断。
5. **`✅ 无违规`** → 一句话报告通过。
