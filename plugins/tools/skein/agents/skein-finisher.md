---
name: skein-finisher
description: SKEIN finish 阶段收尾勘察器。被 main 派发, 在 task worktree 内读 git diff, 回传收尾摘要 (本 task 改了什么) + subtask 逐条完成核对 + 悬挂后台任务清单。只读 (无 Write/Edit), 不跑 sediment (归 skein-memorier), 无 Agent/Task (Recursion Guard)。
