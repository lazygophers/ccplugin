...
```bash
# 确定固定点：task worktree 的 merge-base 或 task 创建时的 commit
git log --oneline -1
git diff <fixed-point>...HEAD   # 三点 diff
git log <fixed-point>..HEAD --oneline
```

fixed-point 取 task worktree 分支从 main 分叉的点。若无法确定，取 `HEAD~10` 或问 main。
...
- **工具失败标 `[工具失败: <原因>]`** — git/diff 命令报错时标记，不当空 diff 处理。
