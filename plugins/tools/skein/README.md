...
| 看板 (文本 + 可视化) | `skein.py board` / `view` | `.skein/task.md` 文本看板 + `.skein/task.html` 可视化页 (静态壳, render.js 运行时 fetch json 渲染; 预估进度总览 + 执行顺序图 + 每 task 时间条, 4 主题 6 配色 深浅色, 页内切换器)。`view` 起本地 http server 打开 |
...
├── task.html          # 可视化看板静态壳 (render.js 运行时 fetch json 渲染, git 忽略, `skein view` 起本地 server 打开)
