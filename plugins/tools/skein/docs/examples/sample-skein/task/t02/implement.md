...
```bash
skein.py subtask add t02 s1 --name "请求参数校验" --write "internal/order/validate*.go" --reason "..."
skein.py subtask add t02 s2 --name "库存扣减"     --write "internal/inventory/*.go"    --reason "..."
skein.py subtask add t02 s3 --name "订单落库"     --write "internal/order/repo*.go"     --reason "..."
skein.py subtask add t02 s4 --name "订单创建事件" --deps "s3" --write "internal/event/*.go" --reason "..."
```
