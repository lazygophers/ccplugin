...
一份**执行中途**的真实 `.skein/` 快照, 对着 [glossary.md](../glossary.md) / [workflow.md](../workflow.md) 看每个文件长啥样、谁维护, 以及跑起来后 `.skein/` 目录里的**实际内容**。全部 json/md 由 `skein.py` / `memory.py` 真跑生成 (非手搓), 仅把 worktree 绝对路径洗成 `/your/repo/` 占位。
...
| task | 状态 | 说明 | 落盘位置 |
| --- | --- | --- | --- |
| t01 订单查询接口 | **completed** (已归档) | 走完 plan→exec→check→finish, worktree 已合并销毁 | `task/archive/2026/07-11/t01/` |
| **t02 订单创建 API** | **in_progress** (focus) | planning 完, exec 中途 — subtask 覆盖四态 | `task/t02/` |
| t03 订单支付 | **pending** | 仅 `create` 排队, 未 start, 无 worktree / 无 subtask | `task/t03/` |

focus = t02 (最近 start), 见顶层 `task.json`。t02 的四个 subtask 恰好各占一态:

| sid | 名称 | 状态 | 含义 |
| --- | --- | --- | --- |
| s1 | 请求参数校验 | **done** | 已完成 |
| s2 | 库存扣减 | **running** | 正在跑 (占一个并发槽) |
| s3 | 订单落库 | **failed** | 首跑失败 (幂等键冲突), 待重试 |
| s4 | 订单创建事件 | **pending** | 依赖 s3, s3 done 前不就绪 |

> 这正是 `subtask claim` 跑到一半的样子: 首轮认领 s1+s2 (并发 2), s1 先 done、s3 补位后 fail; 真实调度环下一步会 `subtask start t02 s3` 重试, 成功后 s4 才 ready。
...
| `task.json` | 顶层状态 `{focus: "t02"}` | **脚本** · AI 禁读写 (guard 硬阻) |
| `task.md` | 顶层看板 (t02 in_progress / t03 pending, 由 task.json 渲染) | **脚本** · AI 禁读写 |
...
python3 <skein>/scripts/skein.py current --all      # 看 active task (t02 focus)
