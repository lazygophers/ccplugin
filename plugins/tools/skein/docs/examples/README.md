...
| `task.html` | 可视化看板 (title/标题带项目名, 2 列 dashboard + 满宽概览 banner): 任务进展总览 (预期/已耗/剩余合计 + 综合 & 预估加权完成率 + 预计执行顺序图) + 每 task 耗时/预期时间条 + 子任务完成% + 「展开明细」折叠区 (subtask DAG + 子任务表默认收起保紧凑, 点开看全量) (4 主题 6 配色 深浅色, 页内切换器, 由 task.json 渲染) | **脚本** · `skein.py view` 打开 |
