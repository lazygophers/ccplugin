...
> SessionStart 时若 git 仓无 `.skein/`, hook 会**自动注入** setup 建议; Claude 主动调 **setup** skill 完成初始化, 无需手动。
...
看板文件 `.skein/task.md` (文本) + `.skein/task.html` (可视化) 每个生命周期节点后自动重渲染。**禁直接编辑 task.md** — guard hook 会硬阻; `task.html` 只读, 用 `view` 按需打开。
