...
- **Python 3** — 引擎 `skein` / `skein-spec` 是纯 Python 3 脚本, 无第三方依赖。
...
> plugin 启用后 `bin/` 自动进 Bash tool 的 PATH, 直接用裸命令 `skein` / `skein-spec` 即可; 下文示例均用此短形式。
...
> **推荐入口 = `setup`** (不是裸 `init`): 幂等 scaffold + trellis 迁移。检测到 `.trellis/` → **独立拷** spec 入 `.skein/spec` + **物理迁移 task.json 与各 task 文件夹** (整体搬迁, 跳过已归档 task) + **无条件删 trellis 接线** (hooks/scripts/settings + `.claude/*trellis*`, 避免双注入) + **在 `.claude/settings.local.json` 禁 trellisx 插件** (防插件级双注入) + 输出迁移 manifest。默认**兼容模式**留 `.trellis/` 数据; 加 `--full` 整删 `.trellis/`。spec 重组交 `skein-setup` agent 语义迁移。无 trellis → 等价 `init` + 建本地 spec 库。
...
> **可选 — 空仓冷启动播种**: 若是已有代码但从没用过 SKEIN 的仓库 (`.skein/spec` 为空), 可先走 `skein-spec` 冷启动播种 (`references/bootstrap-seeding.md`) — 扫既有代码库约定 (命名 / 错误处理 / 测试 / 架构边界 / 构建) 播种一批基线规则, 让第一个 task 就能召回项目习惯。一次性动作, 之后靠正常 finish 增量沉淀。首次上手不跑也行。
...
3. **check** — 派 checker 跑 lint / type / test / 契约校验, 不过就派合适 agent (无则 `skein-executor`) 修。
...
| 边界模糊 (走 flow vs inline 难判) | Claude 自行裁定, 默认偏保守走 flow; 仅极不确定 (破坏性/远超预期) 才用 AskUserQuestion 问你 |
