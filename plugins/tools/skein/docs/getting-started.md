...
├── task.html        # 可视化看板静态壳 — render.js 运行时 fetch task.json 客户端渲染 (4 主题 6 配色 深浅色, git 忽略); `skein view` 起本地 server 打开
├── board/           # 静态壳 index.html + render.js + switcher.js + 主题/配色 CSS + view-config.json (从插件 assets 拷贝, git 忽略)
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/skein.py view       # 起本地 http server 打开 task.html 看板 (Ctrl-C 停)
```

看板文件 `.skein/task.md` (文本) 每个生命周期节点后自动重渲染; `.skein/task.html` 是静态壳, 数据由 `render.js` 运行时 fetch `task.json` 客户端渲染 (变更无需重渲 html)。**禁直接编辑 task.md** — guard hook 会硬阻。task.html 须经 `view` 起本地 server 打开 (`file://` 双击 fetch 被浏览器 CORS 拦)。
