...
| `skein list [--status <态>] [--json]`                                             | 列 task。`--status` 取 待处理/调研中/进行中/检查中/收尾中/已完成 (或 pending/research/active/check/finishing/done), `open`=全部未完成, 逗号多选; `skein list --status open` 可列全部活跃 task。`--json` 固定返回对象信封 `{"tasks": [{id,status,name,desc,deps,repos,worktree,worktrees,priority,pct,subs,ready}, ...]}`；`status` 固定是英文枚举 `pending\|research\|active\|check\|finishing\|done` (中文名只用于看板展示与 `--status` 入参, 不出现在 `--json` 输出里)；消费方从 `.tasks[]` 读取、取单个 tid 用 `jq -r '.tasks[0].id'`，禁止按裸数组 `.[]` 解析或与中文展示名比较。 |
| `skein board`                                                                     | 文本看板                                                                                                                                                                                                                                   |
