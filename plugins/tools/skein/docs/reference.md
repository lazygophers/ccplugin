...
| `skein status [--pretty]`                                                        | 全局运行态概览: work/gate 两池占用 + 执行中 subtask + 就绪待派计数 + 状态统计。默认 JSON 精简形态: `running_subtasks[]` 只含 `{tid,sid,name,status}`, `active_tasks/plan_tasks/gate_tasks[]` 只含 `{id,name,status}` (调度细节走 `--pretty` 或 `flow run --dry-run`); `--pretty` rich 渲染含阶段/进度/已跑/工时/依赖阻塞等完整细节。单 task 详情仍走 `skein task status <tid>` |
| `skein board`                                                                     | 文本看板                                                                                                                                                                                                                                   |
