...
- **无入参 (空)** → 不建新 task, **驱动 `.skein` 内既有 task 走完整闭环直到 finish** (不止 exec):
  ...
  3. 有 → 逐个加载 `skein-flow` **走完整闭环** (task 级并发受 `max_active` 默认 2 限, ready 即启 / 完成即启 / 冲突或 `depends_on` 未满足则串行等 `skein start`; **🛑 deps 仅阻塞 start, 不阻塞 plan/confirm — pending task 不论前置是否 plan/finish, 一律提前 plan 到就绪, 仅 `skein start` 才等前置**):
...
> **工作目录 (worktree 态自适应)** — 详见 skein-workflow/references/worktree-convention.md。真值一律以 task 的 `worktree` 字段为准 (null=原地)。

main 作调度器编排, 改动落各 subtask 工作目录 (worktree 或原地仓库根)、每个 agent 完成即回传。角色分工:

- **🛑 subtask 状态先行 (硬前置)** — 详见 skein-workflow/references/state-before-action.md 硬门 2。claim 占槽是派 agent 的硬前置，pending/failed 态 subtask 禁直接派 agent，必须先标 running 占槽后才能 dispatch。
...
- **🟢 subtask 调度优先, 空闲才提前 plan (禁干等)** — 详见 skein-workflow/references/dag-scheduling.md 第 6 节。优先级: 有可调度 subtask → 一律先 `claim` 派 subtask (尽早完成在飞 task), plan-ahead 是次级填充器。plan-ahead 必须让位 subtask, 每步 planning 前/后回探 `claim`, 一旦有 subtask 可派立即放下 planning 回去派。
- **🔴 exec 无验收 (完成即 done, 验收全归 check)** — subagent 回传即执行完成, main **只 `done`/`fail`, 禁 exec 阶段勾验收** (`subtask check` 勾验收/checkpoint 核对归 `skein-check` 阶段)。`done` = 执行动作完成; subtask 的 `--check` 验收项由 skein-check 统一核对。exec 只判「执行有没有跑完/报错」, 不判「验收过没过」。

- **并行只看 depends_on DAG / 并发上限 2 / 完成即派** — 详见 skein-workflow/references/dag-scheduling.md。ready = 所有前置 done + 有空闲并发槽, 无写文件冲突自算; 任一返回即 `done` 后再 `claim`, 不等一批跑完。
...
- **subtask 失败 → 自愈闭环 (禁失败即停摆)** — 详见 skein-workflow/references/subtask-operations.md 第 3 节。subtask 报错/验收不过, main 读根因自主修, 二选一 (均在本 task scope 内): ① 定点小缺陷 → 原地重派 ≤2 轮; ② 根因是独立可修单元 → 插修复 subtask 定点修根因后重派。兜底: 修复也失败/累计无进展超上限/根因超 scope → 停回传 (走 skein-check root-cause-protocol 或转人工)。禁跳过该 subtask 放行下游。
...
subtask 级 + 多 task 级两层同构 (同一套 DAG), subtask 状态经 `skein subtask` 脚本落盘, dispatch prompt 6 字段自包含 (含 Recursion Guard + 读后写硬门)。完整 DAG 调度算法详见 skein-workflow/references/dag-scheduling.md；subtask 状态机详见 skein-workflow/references/subtask-state-machine.md；worktree 规则详见 skein-workflow/references/worktree-convention.md。
