...
- **调度** → main 亲跑 (脚本不能 spawn): `skein subtask claim <tid>` 算就绪批 + 标 running, main 逐个真实 `Agent` 调用 dispatch。批量推进用 `claim`; 只想先看一个可执行候选再决定是否执行, 用 `skein pop` (只读提取一个 (task, subtask) 对, 不改态)。
...
- **及早退出 (subagent / workflow 皆适用)** — 每个载体只做本 subtask、产出即回传**立即退出**, 禁滞留空转 / 轮询等待 / 揽额外活。main 侧 `done` 后即 `claim` 放行下游 (完成即派), 全部 done 立即收束进 check, 禁挂着不结。

## 调度循环 (动态, 完成即派)

```
while skein subtask claim <tid> 返回非空:       # 脚本一步: 算就绪 + 标 running (≤ max_parallel)
    对认领到的每个 subtask: 为其选合适 agent (无则 skein-executor) 真实 Agent 调用
    等任一 subagent 返回
    → skein subtask check/done/fail <tid> <sid> → 回到 claim (脚本自动重算就绪, 完成即派)
```
