...
> **复用不新造**: 扫描仍用 `skein-researcher` (bootstrap 扫描模式), 写盘仍走 sediment 审批门。重构**只多两步**: 前置 `memory.py archive` (可逆清库) + 后置按项目类型分型扫描。**唯一新脚本能力 = archive/restore**, 其余零改。
...
⑤ 逐条判层 → ⑥ sediment 审批写盘 → ⑦ 验证 + 保留归档(供回滚)
```

### ① 快照现状 (审计基线)
```
python3 <plugin>/scripts/memory.py list          # 记下重构前两层条数/文件
```
留一句话交代重构动机 (哪种失效/漂移), 便于日后审计。

### ② 可逆归档 (清库, 非删除)
```
python3 <plugin>/scripts/memory.py archive       # 两层全归档到 .skein/spec/.archive/<ts>/
...
### ⑥ sediment 审批写盘 (main 亲做)
逐条走 sediment 审批门 ([sediment-workflow.md](sediment-workflow.md)), `--source reconstruct` 统一标来源。**未过审批禁写盘**。用户可逐条批/驳/改层。

### ⑦ 验证 + 保留归档
```
python3 <plugin>/scripts/memory.py list          # 对比重构前后条数/分层合理性
```
- **归档目录保留** (不删), 作回滚锚点 + 审计证据。确认新库无误后可手动清 `.archive/<ts>/`。
- 回滚: `python3 <plugin>/scripts/memory.py restore <ts>` (撞名旧规则加 `restored-` 前缀并存, 不覆盖新库)。
...
| 情况 | 处理 |
|---|---|
| 归档后重建不满意 (漏规则/判层错) | `python3 memory.py restore <ts>` 恢复旧库, 撞名新规则加 `restored-` 前缀并存, 再手动取舍 |
...
| 为重构大改 memory.py | 只用新增 archive/restore, 扫描/写盘复用 researcher + sediment |
