...
> **复用不新造**: 扫描仍用 `skein-researcher` (bootstrap 扫描模式), 写盘仍走 sediment 写盘流程。重构**只多两步**: 前置 `skein-memory archive` (可逆清库) + 后置按项目类型分型扫描。**唯一新脚本能力 = archive/restore**, 其余零改。
...
### ① 快照现状 (审计基线)
```
skein-memory list          # 记下重构前两层条数/文件
```
...
### ⑥ sediment 写盘 (main)
逐条走 sediment 写盘 ([sediment-workflow.md](sediment-workflow.md)), `--source reconstruct` 统一标来源。重构跑前 (②前) 已一次 `AskUserQuestion` 征同意 (归档虽可逆仍全局动作), 覆盖整轮 —— 内部候选**自动写盘, 不逐条再问用户**; main 逐项输出 trace 供审阅, 误沉淀可 `skein-memory restore` 回滚归档或后续调层纠正。

### ⑦ 验证 + 保留归档
```
skein-memory list          # 对比重构前后条数/分层合理性
```
- **归档目录保留** (不删), 作回滚锚点 + 审计证据。确认新库无误后可手动清 `.archive/<ts>/`。
- 回滚: `skein-memory restore <ts>` (撞名旧规则加 `restored-` 前缀并存, 不覆盖新库)。
...
| 情况 | 处理 |
|---|---|
| 归档后重建不满意 (漏规则/判层错) | `python3 skein-memory restore <ts>` 恢复旧库, 撞名新规则加 `restored-` 前缀并存, 再手动取舍 |
...
| 为重构大改 skein-memory | 只用新增 archive/restore, 扫描/写盘复用 researcher + sediment |
