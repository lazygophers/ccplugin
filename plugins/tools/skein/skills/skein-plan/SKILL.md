...
5. **产出工件** — `create` 已落 prd/design/findings 三脚手架 (骨架标题, 本步填正文); 调度落 task.json (脚本):
   ...
   - **子任务 + 调度 DAG** — subtask 拆分 (每个含 depends_on + 验收 checklist) 逐条 `skein subtask add <id> <sid> --name --desc --agent [--deps --check]` 落进 task.json (sid/--name/--desc/--agent 四者必填)。**这是 exec 唯一调度真值源**, 不写 mermaid 图文件。
