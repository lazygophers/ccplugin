---
name: skein-cleanup
description: 归档清理 (archive cleanup)。task finish 异常中断、或怀疑残留孤儿 worktree / 悬挂分支 / 漏归档 task 时使用 — 清扫 .worktrees/ 与 skein/* 分支, 把 completed 却没归档的 task 补归档, 让看板与实际一致。安全清扫, 不删未完成任务。
---
...
> ⚠️ 安全第一: 只清「已完成/已合并」的残留。**未 finish 的 active task、未合并的分支一律不删** (可能有未落地改动), 存疑先报用户。
...
3. **漏归档 task** — `skein.py list` 里 `completed` 却仍在 `.skein/tasks/` (未进 archive):
...
| 手动 rm .skein/tasks/<id> 当归档 | 走 `skein.py archive <id>` (进 archive 目录) |
