---
name: skein-bootstrap
description: 冷启动规则播种 (一次性)。仓库首次接入 SKEIN、`.claude/rules` 为空/近空时使用 — 派 skein-researcher 扫既有代码库约定 (命名/错误处理/测试/架构边界/构建) 提炼候选规则, 逐条判 core/recall/drop, 经 sediment 审批门落盘。解决新仓库两层记忆前几十轮无价值的冷启动问题。
---

# skein-bootstrap — 冷启动规则播种
...
- 扫描维度明细见 [references/scan-dimensions.md](references/scan-dimensions.md)。
...
保留下来的候选**逐条**走 sediment 审批门 (**复用** [skein-memory 的 sediment](../skein-memory/references/sediment-workflow.md)):
...
- 想补播 → 手动再调本 skill, 但通常 finish sediment 已够。
