...
ask-matt 的「路由」理念 skein 已内建: skein-flow (强制走 task, 参数路由 plan|exec|check|finish, 默认 plan) 四步闭环自包含 — plan 阶段 (research 判定门 / brainstorm / grill) → exec 阶段 → check 阶段 → finish 阶段。
