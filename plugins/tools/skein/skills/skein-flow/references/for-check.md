...
1. **验证 (经 `Agent` 工具派具名 subagent `skein-checker`, 禁 teammate / team)** — 传 Active task id + 工作目录 (task 的 `worktree` 字段; null=原地仓库根)。checker 自跑状态切换 + checkpoint 核对 (task+subtask 双层, 未勾项写回 `skein prd check`) + 场景自适应内置检查 + 契约逐条核对 + 一致性核查, 全流程权威定义见 `skein-checker.md` (agents/), 本文件不重复。
...
4. **重验 (main 保留项)** — 修复 subtask 全 done 后重派 `skein-checker` 复跑 (含一致性)。未过回 planning 重确认循环。
