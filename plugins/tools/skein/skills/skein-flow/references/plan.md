...
标准七段（顺序固定，`skein state confirm` 硬校验）：`目标` / `边界` / `User Stories` / `验收标准` / `验证方式` / `Testing Decisions` / `索引`。
...
task 与 subtask 各有 `estimate` 字段（小时，浮点，未填为 `null`）。`skein state confirm` 硬校验，缺失或非正数一律拒绝开工。命令用法查 `skein estimate --help`。

#### 铁律

- **plan 阶段必填**：task 工时须在 `skein state confirm` 前填实，与 prd 章节齐备、subtask 拆分同为 confirm 硬门。
...
`skein state confirm` 依次校验：① subtask ≥1 → ② prd 七段齐备无 TODO 占位 → ③ 工时已填实且 ≥ Σ subtask。任一不满足即报错阻断。
