...
1. **派发前先确认本 task 派出的后台 agent 均已结束** (main 职责, 悬挂未清 = 未闭环, 禁派 finisher) — **派发 `Agent(subagent_type="skein:skein-finisher", description=..., prompt=...)`** (禁 teammate / team, 禁传 `team_name` — 形式见 [carrier-rules.md 派发调用形式](carrier-rules.md#派发调用形式-照抄-禁自由发挥)) — prompt 传 `task id + 工作目录` (task 的 `worktree` 字段; null=原地仓库根)。finisher 自主完成勘察改动 (git diff/status) + 在仓库根跑 `skein finish <tid>` (commit→merge→销 worktree→标记完成), 全流程权威定义见 `skein-finisher.md` (agents/), 本文件不重复。
...
| main 确认后台 agent 仍有未结束的 | main 等待其结束或人工中止 | 仍未结束 → 停手, 禁派 finisher (未闭环) |
