...
4. **exec** (agent 编排) — main 作调度器, 动态 DAG 派 `skein-implementer` 各执行 1 subtask, 改动落 task worktree。调度算法 (冲突自算边 ∪ depends_on / ready 即派 / 并发 2 / 完成即派 / dispatch prompt 6 字段) 见 [scheduling-algorithm.md](scheduling-algorithm.md)。每个 agent 完成即回传。
