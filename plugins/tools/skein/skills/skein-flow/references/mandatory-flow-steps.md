...
4. **exec** (agent 编排) — main 作调度器, 动态 DAG 为每个 subtask 选合适 agent (按任务性质挑现有 agent, 无合适的用 `general-purpose`) 各执行 1 subtask, 改动落 task worktree。调度算法 (并行只看 depends_on DAG / ready 即派 / 并发 2 / 完成即派 / dispatch prompt 6 字段携带执行纪律与递归护栏) 见 [scheduling-algorithm.md](scheduling-algorithm.md)。每个 agent 完成即回传。
...
5. **check** (委托 `skein-check`) — 派 `skein-checker` 验证 (lint / type-check / tests / 契约合规); 未过 → 派合适 agent (无则 `general-purpose`) 定点修复重检, 不跳 finish。
