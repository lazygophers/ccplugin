...
仅当仓库原用 trellis (`.trellis/` 存在)。`skein setup` 的 manifest `trellis_present:true` 触发本路径。纯新仓 / 已初始化维护不走这里。
...
scaffold + spec 拷贝 + task 迁移 + 接线删除已由 `skein setup [--full]` 完成, 但 spec 重组 / task 重建 / settings hook 剔除需语义判断 → 派 `skein-setup` agent。dispatch prompt 6 字段自包含：

- **目标**: 把已拷入 `.skein/spec` 的规则语义重组为 `rules/product/map/external` 四 namespace × 类目 (加载策略写进各文件 frontmatter 的 inclusion, 与 namespace 正交) + 重建 task + 剔 settings 里**残留 (非 canonical)** trellis hook 条目。目标结构同 `skein-spec` SKILL.md §namespace×inclusion 正交两维, 非旧 core/recall 两层。
...
- **工作目录与范围**: 仓库根; 改 `.skein/spec` (原地重组进 `rules/product/map/external` 四目录) + `.skein/task/` + `.claude/settings*.json` (JSON 语义剔残留 hook)。
- **输出格式**: 见 skein-setup agent 定义 (namespace/类目分布 + task 迁移数 + 清理清单)。
- **验收标准**: `skein-spec list` 规则均落 `rules/product/map/external` 四 namespace 之一; `skein list` 有迁移 task; `.claude/settings*.json` 无 trellis hook (canonical 已脚本清, 残留已 agent 清); 兼容模式 `.trellis/` 数据仍在 / `--full` 已整删。
...
- **task.md / task.json 禁手改** — 迁移经 `skein create` 等命令, 不直接写 (PreToolUse hook 硬阻)。
