...
|---|---|---|---|---|
| **recall** (轻) | `archive --namespace <长尾 ns>` (其余 namespace 保留) | 五维基线 + **主类型**侧重 (§5) | 仅该 namespace 逐条比对 | 漂移/污染集中长尾, 手工维护的 always 页仍可信 |
| **full** (全) | `archive` 全 namespace 归档 | 五维基线 + **主类型**侧重 (§5) | 全库逐条比对 | 换栈/架构翻新, always 页也过期 |
| **deep** (深) | `archive` 全 namespace 归档 | 五维 + **全 8 型探针深扫** (§5 全表, 非仅主类型) + 项目内容全量 (§3) | 全库逐条 + 归档旧规则**全量核验** | 接手可疑成熟仓/来源不明, 从零核底 |

- **recall** (档名沿用历史, 与 namespace 名无关): 最省, 手工维护的 always 页不动。适合只是长尾 namespace 陈旧/被污染。
...
### ② 可逆归档 (清库, 非删除)
```
skein-spec archive       # full/deep: 全 namespace 归档到 .skein/spec/.archive/<ts>/
skein-spec archive --namespace <ns>  # recall 档: 只归档指定 namespace, 其余保留
```
...
- **always 倾向**: 数据流硬约束 (禁组件内直接 fetch、状态必经 store)、禁硬编码设计值 → core (若团队强约束)。多数前端约定弹性大 → recall。
...
- **always 倾向**: 公共 API 稳定性契约 (禁破坏性变更不升 major)、退出码/流分工 → core。命名/docstring → recall。
...
- **always 倾向**: 可复现硬约束 (种子固定、数据版本锁)、数据隐私禁令 → core。实验记录格式 → recall。
...
- **always 倾向**: 安全硬规 (禁明文密钥、最小权限)、环境隔离禁令、状态锁 → core。命名/标签 → recall (除非合规强制)。
- **规则示例**: `密钥禁明文入 tf/yaml, 走 secrets manager 引用` (always); `禁手动改线上, 一切变更经 plan+apply 审查` (always); `资源必带 owner/env/cost-center 标签` (recall/core 视合规)。
...
- **always 倾向**: 极少进 core — 多为 recall。仅"发布前必过链接检查"类门禁可入 core。
- **规则示例**: `每文档 frontmatter 必含 title/description` (auto); `内链走相对路径, 发布前跑死链检查` (recall/core); `术语统一见 glossary, 禁同义混用` (auto)。
