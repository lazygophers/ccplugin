---
name: skein-finish
description: task 收尾闭环门 (被 skein-flow finish 委托, 或用户显式 "收尾/归档/闭环")。check 全绿后派 skein-finisher 收尾勘察 → skein finish 闭环 (commit→merge→archive→销 worktree) → 异步派 skein-specer 跑 sediment + 检测 .pending-fix 标记双保险派 auto-fix。未 archive = 未闭环, 禁宣告 Done。
user-invocable: true
argument-hint: "[任务ID]"
arguments: "[任务ID]"
model: haiku
...
1. **收尾勘察 (同步门)** — 输入 `task id + 工作目录 (task 的 worktree 字段; null=原地仓库根)` → 派 `skein-finisher` → 出口: diff 摘要 + subtask 完成核对 + 悬挂清单。**未完成 subtask / 未满足验收 → 退回 exec 或 check, 禁放行**。
...
**完成判据 (勾满才算闭环)**:
- [ ] finisher 勘察回传, 无未完成 subtask / 未满足验收
...
| finisher 职责    | finisher 只读勘察, 改码退回 exec、sediment 归 memorier (❌ finisher 自己改码 / 跑 sediment) |
