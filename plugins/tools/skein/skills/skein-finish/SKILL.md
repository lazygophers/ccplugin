...
| sediment 沉淀   | 委托 `skein-memory` (派 memorier) | 草案 → main `skein-memory sediment` 自动写盘 (不逐次询问) |
...
2. **sediment 判定门** — 输入 `diff + exec 各 subagent 回传摘要 (含 SPEC: 标记)` → 委托 `skein-memory` sediment (memorier 跑判定门产 core/recall/drop 草案) → main 逐项输出 trace → `skein-memory sediment` 自动写盘 + reindex (判定门通过即写, 不逐次询问用户; 误沉淀后续调层/删可逆纠正)。**无增量 → 跳过 (禁硬凑)**。
