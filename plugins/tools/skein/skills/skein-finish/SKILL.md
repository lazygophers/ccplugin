---
name: skein-finish
description: task 收尾闭环门。check 全绿后使用 (被 skein-flow finish 委托, 或用户显式 "收尾/归档/闭环") — 派 skein-finisher 收尾勘察 (diff/subtask 核对/悬挂清单, 同步门) → skein finish (commit→merge→archive→销 worktree, 先闭环) → 异步 fire-and-forget 派 skein-memorier 跑 sediment 判定门 + 自主写盘 (不阻塞 finish)
...
| sediment 沉淀   | **异步 fire-and-forget** 派 `skein-memorier` (finish 闭环后) | memorier 自主跑判定门 + `skein-memory sediment` 写盘 + reindex (main 不等回传, finish 已闭环) |
...
4. **sediment (异步 fire-and-forget, finish 后)** — finish 闭环后, 异步派 `skein-memorier` 跑 sediment 判定门 (读 diff + 各 subagent 回传摘要含 `SPEC:` 标记) + **自主** `skein-memory sediment` 写盘 + reindex。main 不等 memorier 回传即结束回合, 回传到达后只补 output trace (finish 已闭环)。**无增量 → memorier 自判跳过 (禁硬凑)**。详见 `skein-memory` skill。
...
- sediment 判定不输出 trace (memorier 回传到达后 main 补 output trace)。
- 未 archive 宣告 Done。
