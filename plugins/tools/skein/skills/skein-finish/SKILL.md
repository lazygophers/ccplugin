---
name: skein-finish
description: task 收尾闭环门 (被 skein-flow finish 委托, 或用户显式 "收尾/归档/闭环")。check 全绿后 (验收已 check 做完) 派 skein-finisher 只勘察改动+悬挂残留 → skein finish 闭环 (commit→merge→销 worktree→标记完成, 异步 spec) → 异步派 skein-specer 跑 sediment + 检测 .pending-fix 标记双保险派 auto-fix。finish 只收尾不做验收核对。未 finish 闭环(标记完成) = 未闭环, 禁宣告 Done。
user-invocable: true
argument-hint: "[任务ID]"
arguments: "[任务ID]"
model: haiku
...
> **工作目录 (worktree 态自适应) + auto_commit 自适应** — 详见 skein-workflow/references/worktree-convention.md。真值以 task 的 `worktree` 字段为准 (null=原地)。`skein finish` 脚本按此自适应。
...
> 🔒 铁律: 未 finish 闭环(标记完成) = 未闭环, 禁宣告 Done; sediment 异步 fire-and-forget 禁阻塞 finish。
...
| sediment 时序    | finish 先闭环 (销 worktree + 标记完成), sediment 异步 fire-and-forget 在后 (❌ sediment 阻塞 finish / 为等 memorier 回传延后 `skein finish`) |
| sediment trace   | memorier 回传到达后 main 补 output trace 供审阅 (❌ sediment 判定不输出 trace) |
| 宣告 Done        | `skein finish` 标记完成后才宣告 Done (归档=保留期后自动迁移) (❌ 未 finish 闭环即宣告 Done) |
