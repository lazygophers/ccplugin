// CLI 转发端点 (/exec /finish) 的失败传播测试。
// 守的是「点确认没反应」那个 bug: 后端 CLI 非零退出时 HTTP 仍是 200 (body.ok=false),
// api.exec 若原样 resolve, 调用方就会当成成功弹「已确认规划」, 而 task 状态压根没动。
//
// 运行 (从 nextjs 项目根目录):
...
async function main() {
  // 1) CLI 失败 (ok=false) → 必须抛, 且带上 stderr 原文供 UI 展示
  {
    ...
    try { await api.exec("confirm", { id: "t1" }); } catch (e) { err = e; }
    ...
    assert(calls.length === 1 && calls[0].includes("/__skein__/exec"), "应真的打了一次 exec 请求");
  }
  ...
  // 2) stderr 为空时不能抛出空串, 要回落到可读的 exit code 文案
  {
    ...
    let msg = "";
    try { await api.exec("del", { id: "t1" }); } catch (e) { msg = (e as ApiError).message; }
  ...
  // 3) CLI 成功 → 原样返回, 不误伤正常路径
  {
    ...
    const r = await api.exec("confirm", { id: "t1" });
