...
import { LayoutDashboard, Network, ListOrdered, CheckSquare, FileText, Archive, Trash2, Settings as SettingsIcon, HelpCircle, Menu, Sun, Moon, Code2 } from "lucide-react";

// 项目名跨页面/重载缓存 —— 避免每次路由切换 + location.reload 后竞态丢失
let _cachedProjName: string | null = null;

async function fetchProjName(): Promise<string> {
  if (_cachedProjName) return _cachedProjName;
  const r = await api.id();
  const parts = String(r).replace(/\/\.skein\/?$/, "").split("/");
  const name = parts[parts.length - 1] || "SKEIN";
  _cachedProjName = name;
  return name;
}
...
const NAV_ITEMS = [
  ...
  { href: "/api-explorer", label: "API", icon: Code2 },
...
export function Sidebar() {
  ...
  const [projName, setProjName] = useState(_cachedProjName || "");

  useEffect(() => {
    fetchProjName().then(setProjName).catch(() => setProjName("SKEIN"));
  }, []);
...
export function Topbar({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  useEffect(() => {
    fetchProjName().then(setProjName).catch(() => {});
  }, []);
