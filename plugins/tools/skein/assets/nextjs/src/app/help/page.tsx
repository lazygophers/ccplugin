...
const TASK_FLOW = [
  {
    ...
    enter: "skein task create",
    exit: "skein task research (发起调研) 或 skein task confirm (人审门通过, 吸收原 start)",
    agent: "main (同步前台)",
  },
  {
    status: "research",
    ...
    enter: "skein task research (须先登记 ≥1 个 --phase research 的 subtask)",
    exit: "skein task plan (收敛调研回规划, 调研中不可直接 confirm)",
  ...
  },
  {
    ...
    enter: "skein task confirm (人审门通过, 吸收原 start)",
    exit: "全部 subtask done → skein task check 或 claim check",
  ...
  },
  {
    ...
    enter: "skein task check 或 claim check",
    exit: "全绿 → skein task finishing 或 claim check (占 pools.gate 槽); FAIL → 回 planning 重确认",
  ...
  },
  {
    ...
    enter: "skein task finishing 或 claim check",
    exit: "skein task finish",
            ...
            <div className="overflow-hidden rounded-lg border border-border/30">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border/20">
                  {[
                    ["skein task create <id>", "新建 task (kebab-case slug)"],
                    ["skein task research <id>", "待处理→调研中: 发起调研"],
                    ["skein task plan <id>", "调研中→待处理: 收敛调研回规划"],
                    ["skein task confirm <id>", "用户确认门 (待处理→进行中, 吸收原 start)"],
                    ...
                    ["skein task finishing <id>", "检查中→收尾中: 占 gate 槽"],
                    ["skein task finish <id>", "收尾: commit→merge→销 worktree→标记完成"],
