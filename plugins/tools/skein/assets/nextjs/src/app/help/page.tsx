...
const TASK_FLOW = [
  {
    ...
    enter: "skein state create",
    exit: "skein state research (发起调研) 或 skein state confirm (人审门通过, 吸收原 start)",
    agent: "main (同步前台)",
  },
  {
    status: "research",
    ...
    enter: "skein state research (须先登记 ≥1 个 --phase research 的 subtask)",
    exit: "skein state plan (收敛调研回规划, 调研中不可直接 confirm)",
  ...
  },
  {
    ...
    enter: "skein state confirm (人审门通过, 吸收原 start)",
    exit: "全部 subtask done → skein state check 或 claim check",
  ...
  },
  {
    ...
    enter: "skein state check 或 claim check",
    exit: "全绿 → skein state finishing 或 claim check (占 pools.gate 槽); FAIL → 回 planning 重确认",
  ...
  },
  {
    ...
    enter: "skein state finishing 或 claim check",
    exit: "skein state finish",
            ...
            <div className="overflow-hidden rounded-lg border border-border/30">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border/20">
                  {[
                    ["skein state create <id>", "新建 task (kebab-case slug)"],
                    ["skein state research <id>", "待处理→调研中: 发起调研"],
                    ["skein state plan <id>", "调研中→待处理: 收敛调研回规划"],
                    ["skein state confirm <id>", "用户确认门 (待处理→进行中, 吸收原 start)"],
                    ...
                    ["skein state finishing <id>", "检查中→收尾中: 占 gate 槽"],
                    ["skein state finish <id>", "收尾: commit→merge→销 worktree→标记完成"],
