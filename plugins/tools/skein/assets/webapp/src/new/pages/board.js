// SKEIN webapp · board 页 (htm + 原生 DOM 重写版)。
// 两栏: 左 sticky 总计+DAG / 右 task 卡片流; card/subtask/SVG DAG/文档弹层; onLive 软刷。
//
// render(mount, params, ctx) 契约: core `[arch] SPA page 模块统一契约`
//   ctx = { api, md, h, onLive, navigate }
//   onLive(cb) → WS 软刷订阅; router 切页自动退订。
//
// ponytail: 沿用旧 board.js 已验证的 HTML 字符串渲染器 (cards + 卡片级 dagHtml + stepper);
//           经 ctx.h() 构造外壳 DOM 后, 用 innerHTML 注入 HTML 串 (含 SVG)。不重造 Sugiyama,
//           不引 petite-vue, 不重写像素级 dagHtml — 那是把验证过的渲染器整段重写, 收益仅换范式。
//           样式随 #view 清空自动移除, 不污染他页。
import { dagHtml, setNodeMaps } from "../../dag.js";

// ── 状态 → CSS 类/变量 映射 (与 _view_board_data 中 st_cls/ss_cls/node_var/node_cls 同源, 硬编码省往返) ──
const ST_CLS = { "待处理": "s-pending", "就绪": "s-pending", "进行中": "s-active", "检查中": "s-check", "已完成": "s-done" };
const SS_CLS = { "待处理": "ss-pending", "运行中": "ss-running", "已完成": "ss-done", "失败": "ss-failed" };

// 组件样式: 从 board/base.css 迁移 (令牌变量已由 dist/app.css / tokens.css 定义, 主题自动生效)。
const BOARD_CSS = `
...
.col-side .dag-wrap{flex:1 1 auto;min-height:0;max-height:none;overflow:auto}
.col-side .dag{max-width:none;height:auto;margin:4px auto 0}
...
.board-skel{padding:64px 24px;text-align:center;color:var(--muted)}
.board-skel .antd-spin{font-size:28px;color:var(--accent)}
.board-err{padding:64px 24px;text-align:center;color:var(--danger)}
`;

// ── 纯渲染函数 (移植自旧 board.js, 已验证; htm 重写不重造像素级 dagHtml/stepper/prd 等) ──
...
// ── 交互: 状态筛选 / DAG 维度切换 / 节点浮层 / 进度条门控 (移植自旧 switcher.js) ──
let filterSet = ["进行中", "检查中", "就绪", "待处理"];
...
function bindContent(layout, io) {
  ...
  // 拦截 DAG 节点 <a href="#task-<id>"> → 滚到对应 card (history 路由后哈希被吞)
...
// ── 文档弹层: 点 .doc-link → fetch /<doc> → ctx.md 渲染 → 弹层 ──
function wireDocModal(mount, modal, ctx) {
  ...
  function onMountClick(e) {
    // 模态销毁后 (mount 不在 DOM 中) 自解绑
    if (!document.body.contains(mount)) { mount.removeEventListener("click", onMountClick); return; }
...
export async function render(mount, params, ctx) {
  ...
  // sticky 左栏 top = 实测 topbar 高 (顶栏无固定高度, 此处补 --topbar 让 sticky 对齐)
  var tb = document.querySelector(".topbar");
  if (tb) document.documentElement.style.setProperty("--topbar", tb.offsetHeight + "px");

  // ponytail: 外壳用 h() 构造真 DOM (htm/h() 渲染契约), 内层大段 HTML 串 + SVG 经 innerHTML 注入。
  //   htm tagged template (ctx.htm) 异步加载, 首屏不可靠 → 直接用同步 h() 取代, 复杂场景按需切。
  const styleEl = h("style", { html: BOARD_CSS });
  const layout = h("div", { class: "layout" });
  const docModal = h("div", { class: "doc-modal", hidden: true },
    h("div", { class: "doc-backdrop" }),
    h("div", { class: "doc-panel" },
      h("div", { class: "doc-head" },
        h("span", { class: "doc-title" }),
        h("button", { type: "button", class: "doc-copy", disabled: true }, "⧉ 复制"),
        h("button", { type: "button", class: "doc-close", "aria-label": "关闭" }, "✕")
      ),
      h("div", { class: "doc-body markdown" })
    )
  );
  const skel = h("div", { class: "board-skel" },
    h("div", { class: "antd-spin", "aria-hidden": "true" }, "◐"),
    h("div", { class: "mt-2 text-sm" }, "加载看板…")
  );
  mount.append(styleEl, layout, docModal, skel);
  ...
  wireDocModal(mount, docModal, ctx);

  var lastDataSig = null;
  async function refresh() {
    var data;
    try {
      // ponytail: /data 单次往返取全量 (cards+overview+filterOpts+stClsMap+ssClsMap+nodeVar+nodeCls)。
      //   /board/fragment + /board/dag 拆端点更细, 但本页要 stClsMap/ssClsMap 渲染 badge, fragment 不返;
      //   硬编码 status→cls 映射亦可, 但同源串更稳 (后端改映射零改前端)。/data 兜底最简。
      data = await api.data();
    } catch (e) {
      skel.classList.remove("board-skel");
      skel.classList.add("board-err");
      skel.innerHTML = "";
      skel.append(h("div", { class: "text-base" }, "加载失败: " + ((e && e.message) || e)));
      return;
    }
  ...
  // onLive 软刷 (WS 全局订阅, router 切页自动退订; 返回 unsubscribe 入 cleanups)
  const unsubLive = ctx.onLive(function () { refresh().catch(function () {}); });
  return [unsubLive];
}
