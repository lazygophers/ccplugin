...
幂等: 可重跑, marker 定位追加块, 零重复。`trellis update` 覆盖后重跑即恢复。
...
1. **AI 层**: 收尾前 `TaskList` 查本 task 名下悬挂后台 agent, 逐个 `TaskStop` 关。
2. **git 层**: `task.py finish` → after_finish hook → `trellisx-finish.py` (commit→merge→销 worktree→archive)。

> 脚本只销 worktree (git)。关后台 Task/agent 是 AI 层职责, 脚本做不到。
...
| 检查 | 期望 |
| --- | --- |
| trellis 原生生命周期规则文件 | apply 未触碰, 零改动 |
...
**Q: apply 会改我已有的 trellis 原生生命周期规则文件吗?**
A: 不会。apply 不触碰它, 只新增 config.yaml hook / spec guide / command; 原生 no_task/Phase/check/finish/前缀全不动。
