...
### subtask 拆分 — 任务内 exec, 并行

- 触发: **任一 task** (含每个 child) 的 exec 含**多独立无影响单元** → `implement.md` 拆 subtask。
- 每个 subtask 派专用 subagent **并行**推 (经 `trellis-implement` 入口)。
...
| exec | trellis-implement → subagent (共享 task worktree) | 源码改动 (落 worktree) |
