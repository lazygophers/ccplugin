...
| `trellisx-finish.py` | 强制收尾 (commit→merge→archive→销 worktree); 多 worktree 映射到同 task 时先合并各子分支 | config.yaml `after_finish` + 手动 CLI |
...
| worktree 隔离 (task 级) | 隔离单位 = **task** (防并发多 task 互相冲突): task.py start 自动建本 task 的 `<git根>/.worktrees/<name>`, archive 销毁。默认一 task 一 worktree; 冲突型并行 subtask 才各开子 worktree (finish 经映射合并各子分支) | workflow.md `in_progress` + config.yaml hook |
