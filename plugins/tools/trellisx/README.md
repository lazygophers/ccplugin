...
| `trellisx-finish.py` | 强制收尾 (commit→merge→archive→销 worktree); 多 worktree 映射到同 task 时先合并各子分支。**只销 worktree (git); 关 Workflow/Task 是 AI 层 (TaskStop), 脚本不做** | config.yaml `after_finish` + 手动 CLI |
...
- **finish 清理**: 收尾前 AI 须确认无悬挂后台 Workflow/agent (TaskList 查、TaskStop 关) —— `trellisx-finish.py` 只销 worktree (git), 关闭 task 名下的 workflow 是 AI 层职责, 脚本做不到。
