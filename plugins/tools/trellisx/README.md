...
5. **`trellisx-spec`** — spec 破坏式优化 (增量捕获走 trellis 原生 trellis-update-spec)。**main 直接执行, 非 fork subagent** (subagent 不能走 AskUserQuestion 审批门)。
6. **`trellisx-cleanup`** — 批量收尾**全部**已完成 task (completed ∪ merged): dry-run → AskUserQuestion → --apply 三段护栏。
7. **`trellisx-grill`** — 对抗式工件审查: 写盘/`task.py start`/spec 重构前, 逐分支 stress-test prd/design/implement/spec, 产弱点表不改写 (源于 grill-me)。路由到 orchestrate/spec 改盘。
