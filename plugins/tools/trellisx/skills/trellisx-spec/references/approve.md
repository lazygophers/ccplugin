...
审批门: 本阶段是强制审批门。强制走 `AskUserQuestion` 工具。**不接受**纯文本"可以"/"OK"。

硬停: 用户未通过 AskUserQuestion 选项明确批准 → 立即停, 禁进入阶段 4 写盘。
...
| 取消 / 否 | 立即停, 报告"0 变更" |
| 用户给文本反馈但未走选项 | 硬停: 重新发 AskUserQuestion, **不**按文本自行决定 |
...
检查点: 用户批准后, 阶段 4 写盘前必须最后跑一次:

```bash
git status .trellis/spec/  # 必须 clean, 否则其他人或 hook 正在改 spec, 等
```

硬停: 非 clean → 报告"spec 目录有未提交改动, 等清干净再执行", 不写盘。
