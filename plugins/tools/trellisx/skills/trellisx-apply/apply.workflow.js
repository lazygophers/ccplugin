export const meta = {
  ...
  phases: [
    { title: 'Plan', detail: '并行: 诊断 + 4 维规划 (read-only, 返回 diff)' },
    { title: 'Write', detail: '备份 → 并行 4 维写盘+自验 → 失败回滚' },
  ],
}

// 维度 (已移除 background 维度)。reference 路径相对 skill 根。
const DIMS = [
  { key: 'workflow', ref: 'references/workflow-injection.md',
    files: '.trellis/workflow.md',
    desc: 'workflow.md 五维 marker 块 (no_task/planning/in_progress/finish_force) + i18n 全文译目标语言 + 删非 Claude Code 平台描述' },
...
if (mode === 'plan') {
  ...
  const planners = [
    () => agent(fields(
      '诊断 .trellis 现状: 首次/更新模式 + workflow-state 锚点块清单 + 已有 trellisx marker + gitignore 状态 + 确定目标语言 (传给写盘)。read-only。',
      '只读 .trellis/**; 禁写盘',
      'StructuredOutput: 复用 PLAN_SCHEMA, key=diagnose, diff 字段填诊断结论 (模式/语言/锚点)',
