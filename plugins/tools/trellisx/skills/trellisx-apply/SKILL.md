...
| **两层拆分 (parent/child + subtask, 概念分清)** | **parent/child (任务级, 依次)**: 多独立可验收交付 → parent + child tasks (`task.py create --parent`), child 是独立 task **依次执行** (按依赖顺序, 非并行), 各完整生命周期; parent 持 child map + 跨 child 验收。**subtask 拆分 (任务内 exec, 并行)**: 任一 task (含每个 child) 的 exec 含多独立无影响单元 → implement.md 拆 subtask, 各派专用 subagent **并行**推 (经 `trellis-implement` 入口, **共享 task worktree, 不传 isolation:worktree**, subtask 与 worktree 无绑定; 多 worktree 仅 opt-in) → trellis-check → finish。**child ≠ subtask** (child 任务级依次 / subtask 任务内并行); child 自身可再 subtask 拆分。main 不直接派 subtask agent / 不直接写源码 | workflow.md `[workflow-state:planning]` + `[workflow-state:in_progress]` 硬规 #2/#3 |
