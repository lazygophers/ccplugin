...
| **worktree 隔离 + 自动收尾 (双路)** | **执行载体 = 1 task : 1 workflow : N worktree** —— task exec 用 Claude Code Workflow 工具编排成 1 个独立 workflow (无 Workflow 平台退 agent 流水线), workflow 内 agent 各 worktree 隔离 (默认 1; 冲突型并行 subtask → N)。worktree: `task.py start` → after_start hook **自动建** `.worktrees/<worktree>`, after_archive 销毁; 收尾**两路都做全链** (commit→merge --no-ff→del worktree→finish→归档): ① **hook 路** `task.py finish` → after_finish hook 跑 `trellisx-finish.py`; ② **finish-work 路** apply 注入目标 `/trellis:finish-work` 令其先跑全链再 journal (见 finish-work 维度)。合并与 worktree 删除必须; 冲突 abort+WARN (AI 须检); 未 archive 禁宣告 Done。**finish 清理边界**: `trellisx-finish.py` 只销 worktree (git); 关闭 task 名下 Workflow/Task 是 AI 层 (TaskStop), 脚本做不到 —— 收尾前 AI MUST 自查 TaskList 无悬挂后台 Workflow/agent | workflow.md 硬规 #4 + **finish 段改写** + config.yaml **after_finish** hook + finish-work.md 注入 + `trellisx-finish.py` |
