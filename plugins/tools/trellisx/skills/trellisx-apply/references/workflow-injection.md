...
- **finish 前确认无悬挂任务 (强制)**: 跑收尾脚本前, 先确认本 task 的 **Workflow / 后台 agent 任务已全部终止** —— 用 `TaskList` 查有无悬挂的后台 Workflow/agent (Claude Code Workflow 或退化的 agent 流水线), 有残留则 `TaskStop` 关闭, 再跑 finish。悬挂任务未清就收尾 = worktree 被合并/销毁时仍有进程在写 = 流程错误。
