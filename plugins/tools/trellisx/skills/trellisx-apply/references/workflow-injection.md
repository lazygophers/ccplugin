...
把 **强推 task** + **subtask 拆分** + **worktree 隔离** + **闭环收尾** 四个维度增量注入 `.trellis/workflow.md`。trellis 原生 `inject-workflow-state` hook 每轮读这些块, 注入内容随之生效。

## 核心原则: 增量追加, 绝不替换原生

**apply 只在 workflow-state 块原生内容之后追加 marker, 绝不替换 / 覆盖 / 重写原生文本。**

- ✅ 注入: `[workflow-state:no_task]` 加**强推 task** / `[workflow-state:planning]` 加 subtask 拆分 / `[workflow-state:in_progress]` 加 worktree 隔离 + 闭环收尾
