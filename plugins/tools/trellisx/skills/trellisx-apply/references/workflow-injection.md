...
🔴 **spec 加载门 (planning 必做, 非软约束; owner = trellisx-orchestrate step 1, flow 不重复加载)**: 进入 planning 后, AI **必须**按 task 主题 grep `.trellis/spec/guides/index.md` 找相关 guide (关键词从 task 标题 / prd 草稿提取), 命中读全文作为 PRD 编排上下文 (约束 / 命令式契约 / 验收基准), 指导 deliverable 矩阵与验收标准。无 `.trellis/spec/` 或无相关 guide → 跳过不报错。**被动可见**: 本 gate 文本由 trellis 原生 `inject-workflow-state` 每轮注入 context (AI 每轮知 spec 存在 + index 路径, 不靠记忆); relevant guide 检索归本步主动 grep (model 驱动)。**诚实边界**: 非全自动按主题加载; 可见性靠 gate 文本借 trellis 原生每轮注入, 相关性检索归 model grep (无独立 per-turn hook 脚本, config.yaml 仅 lifecycle 事件)。目的: planning 即对齐 spec 真值, 避免写完 PRD 才发现违反既有契约。
...
### 注入点 2b: `[workflow-state:in_progress]` 块末尾追加「异步等待清单」(主注入)

**愿景: workflow 异步/后台 sub-agent/审批等待时, AI MUST 输出 4 列任务清单表格**, 让用户每轮看到在跑 sub-agent 的状态摘要。规范主源 = trellisx-orchestrate `progress-communication.md` §异步等待清单格式; apply 把它注入 workflow.md 让目标项目 AI 每轮 (借 trellis 原生 `inject-workflow-state`) 被动看见。

```
<!-- trellisx:start:async-wait-in-progress -->
📌 **异步等待 MUST 输出任务清单 (执行规约)**: 当本 task 处于异步等待 (workflow 异步跑 / 后台 sub-agent 在跑 / 审批等待) 时, AI 每轮回传 MUST 输出 4 列任务清单表格, 让用户即时看到在跑 sub-agent 的进度。**不触发**: 同步前台阻塞 (inline 直做) / 无在跑 sub-agent。

**格式 (4 列, 详见 trellisx-orchestrate `progress-communication.md` §异步等待清单格式)**:

| id | 状态 | 摘要 | 进度% |
| --- | --- | --- | --- |
| <subtask id> | <状态词> | <一句话摘要/阻塞原因> | <完成 subtask ratio, 如 60%; 无数据填 `-`> |

- **列**: `id` · `状态` · `摘要` · `进度%` (4 列, 去 ETA 列因数据不可靠, 阻塞原因合并进摘要)
- **状态本地化** (writer 按目标语言生成状态词): 中文 = `进行中` / `等待中` / `阻塞`; 英文 = `in_flight` / `pending` / `blocked`
- **进度%** = 已完成 subtask / 总 subtask (如 60%), 无数据填 `-`
- 触发场景 / 不触发 / 与进度回传区别 详见 trellisx-orchestrate `progress-communication.md` §异步等待清单格式 (主源, 本注入为被动可见入口)
<!-- trellisx:end:async-wait-in-progress -->
```

## 注入点 3: `[workflow-state:finish]` 段「异步等待清单」(finish 限定语境)

**限定语境**: 仅 workflow 异步跑等 `notification` 回调时 (finish 前等回调, 非 finish 动作本身)。同表不同语境, 内容指向同一 `progress-communication.md` §格式。两 marker key 独立 (与注入点 2 的 `async-wait-in-progress` 不共用), 因语境不同。

```
<!-- trellisx:start:async-wait-finish -->
📌 **等 notification 回调时输出任务清单 (finish 限定)**: 当 workflow 异步跑完成后等 `<task-notification>` 回调 (finish 前等回调) 时, AI 每轮回传 MUST 输出 4 列任务清单表格 (同 in_progress 段异步等待清单, 4 列 `id`/`状态`/`摘要`/`进度%`, 状态本地化)。仅此「等 notification」语境触发; finish 动作本身 (跑 `task.py finish`) 不输出。格式主源 = trellisx-orchestrate `progress-communication.md` §异步等待清单格式。
<!-- trellisx:end:async-wait-finish -->
```
...
- 🔴 **finish 前 spec sediment 判定门 (必做, 非软约束)**: 跑收尾脚本前, AI **按下述 checklist 逐项判**本 task 有无 spec 增量, 任一正向 ✅ → 走 `/trellisx-spec` sediment (提案 → AskUserQuestion 审批 → 写盘 + 同步 index.md) 再 finish; 全否 → 跳过直接 finish。
  ...
  - **诚实边界**: 判定归 AI 非脚本 (语义判断脚本做不了); 全无增量跳过, 禁硬凑沉淀。目的: 学习沉淀不丢, 不为无增量 task 加无谓流程。
...
# 异步等待清单: 注入点 2b (in_progress 块末尾) + 注入点 3 (finish 段), 复用同机制
ASYNC_WAIT = {
    "async-wait-in-progress": (async_wait_in_progress_snippet, "in_progress"),  # 块末尾追加
    "async-wait-finish": (async_wait_finish_snippet, "finish"),                 # finish 段 (无 [workflow-state:finish] 标签时定位收尾段)
}
for tag, snippet in INJECT.items():
    ...
    if start in content:                          # 已注入 -> 替换 marker 内
        ...
        # 块不存在 -> 跳过 (不强行创建, 不破坏原生)
# 异步等待清单注入 (marker 幂等: 同 key 替换, 无则块末尾追加)
for key, (snippet, anchor) in ASYNC_WAIT.items():
    start, end = f"<!-- trellisx:start:{key} -->", f"<!-- trellisx:end:{key} -->"
    if start in content:
        content = re.sub(f"{start}.*?{end}", f"{start}\n{snippet}\n{end}", content, flags=re.DOTALL)
    else:
        if anchor == "in_progress":
            m = re.search(r"(\[workflow-state:in_progress\].*?)(\n\[/workflow-state:in_progress\])", content, re.DOTALL)
            if m:
                content = content[:m.end(1)] + f"\n{start}\n{snippet}\n{end}" + content[m.end(1):]
        else:  # finish 段: 无 [workflow-state:finish] 标签, 定位收尾提醒段 (含 finish-work 的末个 #### 段)
            matches = list(re.finditer(r"(####[^\n]*\n(?:(?!\n####).)*?/?(?:trellis:)?finish-work(?:(?!\n####).)*)", content, re.DOTALL))
            if matches:
                m = matches[-1]
                content = content[:m.end()] + f"\n\n{start}\n{snippet}\n{end}\n" + content[m.end():]
            # 定位不到 -> 跳过 (in_progress 段 async-wait-in-progress 已是权威入口)
