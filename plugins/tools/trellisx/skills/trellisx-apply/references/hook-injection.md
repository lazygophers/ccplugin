...
apply 复制**两个**脚本 (均来自插件 `scripts/`, 统一管理):
...
chmod +x .trellis/scripts/trellisx-worktree.py .trellis/scripts/trellisx-taskmd.py
```

**① `trellisx-worktree.py`** (读 `$TASK_JSON_PATH`, 自适应 3 布局):
