...
apply 复制**六个**文件 (均来自插件 `scripts/`, 统一管理): 四个 hook 脚本 + 两个非 hook CLI 工具 (`trellisx-packages.py` packages 重扫 / `trellisx-cleanup.py` 批量收尾):
...
cp "<plugin-root>/scripts/trellisx-cleanup.py"  .trellis/scripts/trellisx-cleanup.py    # 批量收尾 CLI (非 hook, 见 trellisx-cleanup skill)
chmod +x .trellis/scripts/trellisx-worktree.py .trellis/scripts/trellisx-taskmd.py .trellis/scripts/trellisx-finish.py .trellis/scripts/trellisx-packages.py .trellis/scripts/trellisx-cleanup.py
```
...
- 子命令: `sync <create|start|archive>` (hook 用, 读 $TASK_JSON_PATH 维护确定性列) / `update <tid> --status/--worktree` (AI 用) / `show [tid]` / `cleanup [--days N]`
...
- hook 的 `sync` 维护**确定性列** (ID/名称/描述/状态), upsert 时**保留 AI 细化** (状态细分/worktree) 不覆盖
...
- 三件事: ① 销毁已合并+干净的 worktree (沿用 worktree.py 安全判据, 脏/未合并保留) ② 逐个 `trellisx-finish.py --task <tid>` 归档 (逐个提交) ③ `trellisx-taskmd.py cleanup --days N` 清看板已完成行。
