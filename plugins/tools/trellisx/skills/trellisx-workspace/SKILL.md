...
>
> 🛑 **STOP — 删行前 (AI 手动 cleanup / del / clean)**: 三者均**永久删除看板行**, 属破坏性操作。执行前 **MUST 经 AskUserQuestion 工具确认**删除范围 (`cleanup` 列天数阈值 + 将删 tid; `del` 列目标 tid; `clean` 列将删的孤儿 tid), 用户批准后才删。禁默认静默删除, 禁用"建议清理"软措辞替代确认。(hook 触发的 archive 内 7 天清理是确定性流程, 不走此门。)
