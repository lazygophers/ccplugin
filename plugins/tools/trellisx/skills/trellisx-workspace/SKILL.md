...
| **worktree 创建** (subagent isolation / 手动 git worktree add) | `taskmd.py map-add <worktree> <tid> [备注]` | AI (WorktreeCreate hook 先占位 `?待登记`) |
...
4. **worktree↔task 映射** — `map-add <worktree> <tid> [备注]` 登记 / `map-remove <worktree>` 移除 / `map-check <worktree>` 查 (有→打印 tid 退 0, 无→退 1) / `map-show` 列全部。`WorktreeCreate` hook 创建时自动占位 `?待登记`, `UserPromptSubmit` hook 提醒补全, `WorktreeRemove`/archive 自动清。
5. **规范校验** — `lint` 校验 task.md 是否符合格式规范 (主表 7 列 / 映射区 3 列 / 状态值合法 / ID 不重复); `FileChanged` hook 在 task.md 变更时自动跑, 不合规 → 提醒修。手动: `python3 .trellis/scripts/trellisx-taskmd.py lint`。
...
| 4 | AI 覆盖 hook 维护的确定性列 (ID/名称/描述/状态) | 同行列分工冲突, 互相覆盖 | AI 只改主观列 (阶段/进度/worktree), 保留 hook 列 |
