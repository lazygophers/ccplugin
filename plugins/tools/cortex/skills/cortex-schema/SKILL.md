...
| 模块 | 路径 (相对 `<root>/`) | type |
| --- | --- | --- |
| 项目 | `项目/<host>/<owner>/<repo>/` | `project` |
| 领域 | `领域/<area>/<sub>/[<sub2>/]<topic>.md` (≥ 2 级) | `domain` |
| 脚本 | `脚本/<name>.{sh,py}` (vault 内部, 非用户入口) | `vault-script` |

`<root>` ∈ {`~/.cortex/.wiki`, `<repo>/.wiki`}. 用户操作入口 CLI 在 `~/.cortex/scripts/` (英文), 与 vault 内 `脚本/` (中文) 视觉分离, 禁混用.
