...
18. **wrapper 完整性** — 检 `~/.cortex/scripts/` 下 7 个 wrapper (`lint.sh`, ``, `dashboard.sh`, `doctor.sh`, `install_cron.sh`, `config.sh`, `update.sh`) 是否存在且可执行。缺失 → ⚠️ warn (PR5 才生成), 提示运行 `bash ~/.claude/plugins/marketplaces/ccplugin-market/plugins/tools/cortex/install.sh`
