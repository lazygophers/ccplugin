---
name: cortex-canvas
description: 生成 Obsidian .canvas 文件 (JSON Canvas 1.0), 节点按 Breadcrumbs frontmatter 排布; CLI 不可用时降级写静态 JSON。Triggers on "make canvas", "新建画布", "可视化".
...
1. 解析 vault 路径 (`${CLAUDE_PLUGIN_ROOT}/hooks/_lib/resolve_vault.sh`)
2. 探测 `obsidian` CLI:
...
{
  "nodes": [
    {"id": "n1", "type": "file", "file": "10_concepts/foo.md",
