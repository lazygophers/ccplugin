...
cortex 全部能力以 **19 个 skill** (PR1-4 21→13, 后续增 cortex-config + cortex-image + cortex-image/video/audio-understand 至 18) 暴露, **19 个 slash command** (`/cortex:<name>` 触发 commands/<name>.md, 后者调用对应 skill)。

全部 skill 渐进披露 (D2/D3): SKILL.md 入口 ≤ 80 行 (frontmatter + 触发 + 决策树 + AUTO_MODE 分支 + references 指针), 细节迁 `references/<topic>.md` 2-4 子文件按需加载。

cortex 共 19 个 skill, **~11 自动 / ~7 显式**。显式 skill frontmatter 标 `disable-model-invocation: true`, 不进 description 池, 主线模型不会自动激活, 必须用户明确请求 (中文短语 / 英文触发词 / slash 命令 / wrapper bash)。
...
| `cortex-dataview`         | 自动 ("dataview" / "DQL" / "查询块" / "dv.pages")  |
...
cortex 提供 **6 个专用 agent** (sub-agent, PR4 删 linker: 7→6), 在 19 个 skill 之上提供"接任务后多轮自主推进"能力。agent 与 skill 互补, 不替代。每个 agent 头部含 "vs <skill>" 分工注释行明确边界。
