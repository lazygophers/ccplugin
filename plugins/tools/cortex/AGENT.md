...
cortex 共 13 个 skill, **~8 自动 / ~5 显式**。显式 skill frontmatter 标 `disable-model-invocation: true`, 不进 description 池, 主线模型不会自动激活, 必须用户明确请求 (中文短语 / 英文触发词 / slash 命令 / wrapper bash)。
...
cortex 提供 **6 个专用 agent** (sub-agent, PR4 删 linker: 7→6), 在 13 个 skill 之上提供"接任务后多轮自主推进"能力。agent 与 skill 互补, 不替代。每个 agent 头部含 "vs <skill>" 分工注释行明确边界。
...
bash wrappers 由 `scripts/install_wrappers.sh` 安装到 `~/.cortex/scripts/` (24 个: 10 slash + 3 shell + 11 CLI), 内部 `exec python3 $PLUGIN_ROOT/scripts/cli/<name>.py "$@"`, 0 算法回退 (业务逻辑 100% 保留, 仅删 MCP 协议层)。`migrate.sh` 是一次性脚本, 不计入 24 集。
