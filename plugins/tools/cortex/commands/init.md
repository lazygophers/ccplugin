...
4. 按 `presets/_structure.json` 创建知识库 + 记忆 + 仪表盘 + 归档目录树
5. 复制 seed_files (含占位符: `{{TITLE}} {{CURRENT_PATH}} {{LAST_UPDATED}}`)
6. **跳过** git auto-sync 询问 (默认 off)
7. **跳过** cron 注册询问 (用户单独跑 `/cortex:install_cron`)
