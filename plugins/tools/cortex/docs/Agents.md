...
| agent | 范围 | 角色 | 核心调度 |
|-------|-----|------|----------|
| `cortex-curator` | 知识库 | 维护员 — 解读 lint 报告, 提修复方案 (proposal-only) | cortex-lint → 解读 → cortex-refactor 提议 |
| `cortex-researcher` | 知识库+外网 | 研究员 — 多 source 抓取 + 入库 + 综述 | cortex-search → defuddle → cortex-ingest × N → cortex-summarizer |
| `cortex-translator` | 知识库 | 译者 — 跨 lang 翻译副本, 保 wikilink | cortex-search → 翻译 → cortex-save (新 lang 副本) |
| `cortex-historian` | 知识库+记忆层 | 史官 — 多月份 sessions → folds 季度叙事 | cortex-session → cortex-summarizer → cortex-fold |
| `cortex-cartographer` | 知识库 | 制图员 — canvas + dashboard 二件套 | cortex-canvas → cortex-dashboard → dashboard patch |
| `cortex-archivist` | 知识库 | 档案员 — 收件箱/ 老化迁移提案 | cortex-search → cortex-refactor (move) |
| `cortex-linker` | 知识库 | 连接员 — SC 找近邻, 提议增链 | SC API → cortex-search → cortex-save (patch wikilinks) |
| `cortex-summarizer` | 知识库 | 总结员 — TL;DR callout 注入 | 读页 → 主模型 → patch 页头 |
