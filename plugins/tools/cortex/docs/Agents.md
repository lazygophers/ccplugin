...
cortex 引入 **6 个**专用 agent (PR4 删 linker: 7→6), 在 15 个 skill 之上提供"接任务后自主跑多轮"能力。每个 agent 单 .md 文件, 走 Claude Code 标准 sub-agent 协议。
...
| `cortex-summarizer` | 知识库 | vs cortex-digest: 受调度产单页摘要 vs 8 阶段全量深度 daily pipeline | 读页 → 主模型 → patch 页头 callout |
