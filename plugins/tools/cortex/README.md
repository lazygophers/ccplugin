...
- **CLI 主, MCP 兜底** — 官方 `obsidian` CLI (v1.12.4+, 需 Obsidian app 在跑) 覆盖 read/write/list/search/move/property/daily; CLI 无法表达的场景 (heading/block 锚点 patch、canvas/非 md) 回退 mcp__obsidian__*; 仍兜底不上才直接写文件 (须 AskUserQuestion 授权)。
