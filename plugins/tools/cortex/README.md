...
| 后台 Worker | `agents/cortex-{lint,history,evolve,extract,ingest}-worker.md` | `background: true`; 被对应 skill `context: fork` 启动; 只读扫描→出 plan (不 ask/不落盘, 留主会话) |
...
| 脚本 `ingest.sh` | `scripts/ingest.sh` | GitHub/GitLab/Website/local 输入识别 + 路由 + dry-run plan |
| 脚本 `lint.sh` | `scripts/lint.sh` | 7 规则校验 + R2/R4 autofix |
| 脚本 `extract.sh` | `scripts/extract.sh` | L4-inbox → L1/L2/L3 + projects/domains 路由 |
| 脚本 `history-digest.sh` | `scripts/history-digest.sh` | 扫 ~/.claude/projects jsonl → 学习增量 dry-run plan |
