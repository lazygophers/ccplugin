...
| Skills | 13 | 自动 / 显式触发, 全部多文件渐进披露 (入口 SKILL.md ≤ 80 行 + `references/`) |
...
| `digest.sh` | 📚 | 6 阶段 daily pipeline (读+析+处+更新+清理+evolution 抽取) |
...
| `cortex-summarizer` | "总结这页 / 这个领域" / 被 researcher / cartographer 调度作终产物 | vs `cortex-digest`: summarizer 即时单页摘要 (patch 页头 callout); digest 5 阶段 + 路由 + 评分 |
...
| 03:00 daily | `digest.sh` | 6 阶段 log/session 处理 + evolution 抽取 | ✅ |
...
- [Skills 详解](docs/Skills%20详解.md) — 13 skill 用途 / 触发 / 示例 / 失败处理
