...
1. **请求分类** — 判定属哪类操作:
   - 查/搜知识库 (查不到主动补) → 调 `cortex-recall` skill (搜双层 vault → 兜底 → 回填)
...
3. **执行** — 默认 apply/--fix 直接落盘 (lint --fix / extract --apply); `--dry-run` / `--check` 作 opt-in 预览, 只出 plan 不落盘
...
默认 apply 会直接改写 vault, 报告中显著提示用户; 需预览时用 `--dry-run` / `--check`:
- `lint --fix` 改 frontmatter / mkdir → 默认直接落盘
- `extract --apply` 移动 inbox 文件 → 默认直接落盘
- 写入 L0-core → 默认自动落盘 (不再 ask)
- forget 候选 → 仅标记, 不自动删
...
| `cortex-recall` | 查/搜知识库 / recall / 想想 / 记得 / 查资料 | 答案 + 引用; 未命中兜底 + 回填 |
| `cortex-extract` | extract / 提取 / promote / 整理 / inbox / digest | 归档 plan |
| `cortex-history-digest` | 整理 Claude Code 历史 / digest transcripts / ~/.claude/projects | 全局记忆 plan |
| `cortex-context-digest` | 整理上下文 / 沉淀本次会话 / digest context / 任务收尾 | 项目级或全局记忆 plan |
| `cortex-evolve` | 升级记忆 / 降级 / promote / demote / 金字塔维护 / 记忆 audit | 升降级 plan (L4-L2 自动; L1/L0 仅升) |
