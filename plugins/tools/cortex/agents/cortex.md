...
1. **请求分类** — 判定属哪类操作:
   - 入库/归档 → 调 `cortex-schema-knowledge` skill 决定路径 + `cortex-extract` 落盘
   - 写入记忆 → 调 `cortex-schema-memory` skill 决定等级 + 写 `memory/L<n>-<suffix>/`
   - vault 体检 → 调 `cortex-lint` skill
   - L4-inbox 整理 → 调 `cortex-extract` skill
...
| skill | 触发 | 输出 |
| --- | --- | --- |
| `cortex-schema-knowledge` | 入库 / 三模块路径 / projects/domains/scripts | 路径决策 |
| `cortex-schema-memory` | 记忆等级 / L0-L4 / 永远 / 暂时 / 记住 / 遗忘 | 等级决策 |
