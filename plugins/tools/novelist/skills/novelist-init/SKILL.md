...
1. **取参数** — 从用户输入取「小说名」(必需) 与「类型」(可选, 如 玄幻/科幻/悬疑)。缺小说名 → 硬停, 用 `AskUserQuestion` 问, 禁自行编名。
2. **定父目录** — 默认当前工作目录。若用户指定路径则用之。
3. **审批门 — 确认创建参数** — 执行脚本前向用户汇报: 小说名 / 父目录绝对路径 / 类型(若有)。用 `AskUserQuestion` 确认「确认创建 / 取消」。用户取消 → 退出, 不建目录。
4. **跑脚手架** — 执行:
   ```bash
   # 有类型
   ...
   ```
   脚本创建: `人物/ 世界观/ 设定/ 大纲/ 情节/ 章节/ 元数据/` + 各层模板与索引文件。硬停: 脚本退出码 2(目标已存在且非空) → **禁直接加 `--force` 覆盖**, 先用 `AskUserQuestion` 问用户「换名 / 追加 / 取消」, 用户选追加才补 `--force`。
...
| 用户在 审批门 选「取消」 | 退出, 不建目录, 告知用户可重新调用 | — |
