export const meta = {
  ...
  description: '逐章串行批量写小说: 前置(路线图→世界观→预检)→逐章(write→check→humanize→proofread→fix循环→定稿, 前章定稿才写下一章)→统一检查。root 与设定全部参数化/从小说文件读, 不硬编码任何小说。',
  phases: [
    { title: '大纲', detail: '路线图→世界观更新→一致性预检' },
    { title: '写作', detail: '逐章串行 write→check→humanize→proofread→fix循环→定稿, 前章定稿才下一章' },
...
if (!ROOT) {
  throw new Error('novelist-pipeline: 缺 args.root(小说项目根目录绝对路径)。pipeline 不硬编码任何小说路径。')
}
const START = _args.startChapter || 1
const END = _args.endChapter
...
function ch(n) { return String(n).padStart(3, '0') }
...
function computeScores(checkResult, proofResult, humanResult) {
  ...
  const total = Math.round((cScore * 0.5 + tScore * 0.2 + hScore * 0.3) * 10) / 10
...
async function ensureRouteMap(batchStart, batchEnd) {
  ...
  try {
    existing = await agent(
      ...
      { label: `route-check:${batchId}`, phase: '大纲', model: 'haiku', effort: 'low' }
    )
  } catch (e) { log(`路线文件读取失败: ${e.message}`) }

  if (existing && typeof existing === 'string' && !existing.includes('NOT_FOUND')) {
  ...
  if (!routeMap) { log(`⚠️ 路线图 ${batchId} 生成失败, 跳过本批`); return null }
  return routeMap
}

function parseRouteMap(routeText) {
  const chapters = {}
  if (!routeText || typeof routeText !== 'string') { log('⚠️ 路线图为空, 无法解析'); return chapters }
  ...
  for (let i = 1; i < blocks.length; i += 2) {
    ...
    const foreshadow = block.match(/伏笔推进[：:]\s*(.+)/)?.[1]?.trim() || ''
    const target = parseInt(block.match(/字数目标[：:]\s*~?(\d+)/)?.[1]) || 3000
...
async function fixer(chNum, title, checkResult, proofResult, humanResult) {
  ...
  const needsTextFix = proofResult?.includes('逻辑矛盾') || proofResult?.includes('硬伤')
  const needsHumanFix = humanResult?.includes('中') || humanResult?.includes('重')

  const fixers = []
  if (needsCheckFix) fixers.push(() => agent(
    `修复一致性问题。读 ${file}、${ROOT}/世界观/规则.md。\n问题: ${checkResult?.slice(0, 500)}\n只修冲突点, 不改风格。直接改文件。`,
    { label: `fix-c:${num}`, phase: 'Fix', model: 'sonnet' }))
  if (needsTextFix) fixers.push(() => agent(
    `修复文字硬伤。读 ${file}。\n问题: ${proofResult?.slice(0, 500)}\n只修硬伤, 不改风格。直接改文件。`,
    { label: `fix-t:${num}`, phase: 'Fix', model: 'haiku' }))
  if (needsHumanFix) fixers.push(() => agent(
...
async function finalizer(chNum, title, checkResult, proofResult, humanResult) {
  const num = ch(chNum)
  const { cScore, tScore, hScore, total } = computeScores(checkResult, proofResult, humanResult)
  const passed = total >= PASS_TOTAL && cScore >= PASS_CONSISTENCY && hScore >= PASS_HUMANNESS
...
async function unifiedCheck(chapterNums, chapters) {
  ...
  const results = await parallel(chapterNums.map(n => () => agent(
    `对第${ch(n)}章做最终全局一致性检查。\n\n` +
    ...
    `输出: 通过 / 有冲突(附问题清单)`,
    { label: `unified:${ch(n)}`, phase: '审查', model: 'sonnet' }
  )))
  const conflicts = []
  results.forEach((r, i) => { if (r && r.includes('冲突')) conflicts.push(chapterNums[i]) })
  if (conflicts.length) log(`⚠️ 统一检查发现冲突: 第${conflicts.map(ch).join(', ')}章`)
  else log(`✅ 统一检查全部通过`)
  return { conflicts, results }
}

// ===== 主流程 =====
log(`流水线启动: 第${ch(START)}-${ch(END)}章, root=${ROOT}, 每批${BATCH_SIZE}章`)
const allResults = []
...
for (let batchStart = START; batchStart <= END; batchStart += BATCH_SIZE) {
  ...
  if (!routeText) { log(`⚠️ 批次 ${batchId} 路线图缺失, 跳过`); continue }
  const chapters = parseRouteMap(routeText)
  const chapterNums = Object.keys(chapters).map(Number).filter(n => n >= batchStart && n <= batchEnd)
  if (chapterNums.length === 0) { log(`⚠️ 批次 ${batchId} 无章节信息, 跳过`); continue }
  ...
  for (const n of chapterNums) {
    ...
    let checkResult, humanResult, proofResult, attempts = 0
    while (true) {
      ...
      const { cScore, tScore, hScore, total } = computeScores(checkResult, proofResult, humanResult)
      log(`第${ch(n)}章评分: 一致${cScore} 文字${tScore} 人味${hScore} 综合${total}(第${attempts}次)`)
      const passed = total >= PASS_TOTAL && cScore >= PASS_CONSISTENCY && hScore >= PASS_HUMANNESS
      if (passed || attempts >= MAX_FIX_ATTEMPTS) {
        if (!passed) log(`⚠️ 第${ch(n)}章 超重试上限(${MAX_FIX_ATTEMPTS}), 标记需复审`)
        break
      }
    ...
    allResults.push(r)
    log(`✅ 第${ch(n)}章「${info.title}」${r.passed ? '定稿' : '需复审'} (${r.total}分) → 进入下一章`)
  }
...
if (failed.length) log(`复审章节: ${failed.map(r => `第${r.chapter}章(${r.total}分)`).join(', ')}`)
