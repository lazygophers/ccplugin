...
| 2 | Checker | `novelist:continuity-auditor` | 对照 世界观/规则.md + 人物 + 伏笔 查一致性 | 不管文字/风格 |
| 3 | Humanizer | `novelist:humanizer` | 去 AI 味(匀质/陈词/模板腔) | 不管一致性/错别字 |
| 4 | Proofer | `novelist:proofreader` | 错别字/语法/标点/逻辑矛盾 | 不管一致性/AI味 |
| 5 | Fixer | 对应环 agent | 按三环结果只修问题点(分数不足触发) | 不改风格/不重写 |
| 6 | Finalizer | `novelist:indexer` | 更新索引 + 同步事实源 → 才进下一章 | 不修复/不重写 |

Fixer 跑完回 Checker 重走 check→humanize→proofread, 循环 ≤10 次(`MAX_FIX_ATTEMPTS`)。**本章 Finalizer 定稿后才开始下一章 Writer。** 后置统一检查用 `novelist:continuity-auditor`(只读)。
