...
**综合 >95 且 一致性 >95 → 定稿; 否则退回最弱环修复重评。**(阈值是 `workflow.js` 顶部常量, 可按需调严。)

## 🔴 评分契约(对应 `workflow.js`)

| 分量 | 来源 | 提取规则 | 缺省 | fix 触发 → 修复环 |
|---|---|---|---|---|
| 一致性 cScore | Checker 输出 | 含「冲突」→80; 否则→100 | 100 | 含「冲突」 → fix-c |
...
| 触发 | 一线修复 | 仍失败兜底 |
|---|---|---|
| 综合分 ≤95(分项不足) | 退回最弱环修复重跑 | **无限重试**(`MAX_FIX_ATTEMPTS=Infinity`)直到达标——一直 fix 不停 |
