...
- 编排任务执行顺序，选择合适的 Agent 执行
- 在 `status == running` 时，所有的回复都必须添加 `[MindFlow·${任务内容}·${步骤索引}/${迭代轮数}: ${任务状态}]` 前缀
...
3. **输出示例**：`[MindFlow·${任务内容}·${步骤索引}/${迭代轮数}: ${任务状态}] 开始收集项目信息...`
4. **要点**：
...
3. **输出示例**：`[MindFlow·${任务内容}·${步骤索引}/${迭代轮数}: ${任务状态}] 开始任务分解，识别依赖关系...`
...
2. **输出格式**（必填项：执行流程图（含每个任务 agent/skills/files）、量化验收标准、简要说明）：
   ```markdown
	 [MindFlow·${任务内容}·${步骤索引}/${迭代轮数}: ${任务状态}] 请确认以下执行计划
	 ### 执行流程图（任务队列 + 两槽位并行模型）
...
2. **输出示例**：`[MindFlow·${任务内容}·${步骤索引}/${迭代轮数}: ${任务状态}] 分析失败原因，准备重新规划...`
3. **执行流程**：
