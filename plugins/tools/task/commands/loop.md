...
2. **增量式进展**：
   - 第 1 次迭代：实现核心功能，确保基本可用
   - 第 2 次迭代：完善边界处理，提升稳定性
   - 第 3+ 次迭代：优化性能、改进代码质量、补充文档

3. **持续改进**：
   - 每次迭代验收后，主动审视是否有改进空间
   - 通过 AskUserQuestion 询问用户："是否需要进一步优化？"
   - 即使验收通过，也要考虑代码质量、可维护性等非功能性改进

4. **避免单次完成陷阱**：
   - ❌ 错误：期望第一次就完美实现所有细节
   - ✅ 正确：先确保核心流程通过，再逐步完善
   - ❌ 错误：验收通过后立即结束，不考虑优化
   - ✅ 正确：验收通过后评估改进空间，询问用户意见
...
迭代 2：
- 目标：完善边界处理和异常场景
- 验收：✓ 密码加密、✓ 会话过期处理、✓ SQL注入防护
- 反思：测试覆盖率仅 70%，性能待优化

迭代 3：
- 目标：提升测试覆盖率和性能
- 验收：✓ 测试覆盖率 95%、✓ 登录响应时间 < 100ms
- 反思：代码可读性可进一步提升
