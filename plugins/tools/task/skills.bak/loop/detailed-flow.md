...
| [PromptOptimization](phases/phase-prompt-optimization.md) | 任务规格说明 | 首次必选/用户新输入增量修订/无新输入跳过 | A/B→复杂度评估 / C→重新优化 / 跳过→复杂度评估 |
| [DeepResearch](phases/phase-deep-research.md) | 深入研究（可选） | 复杂度评估+Explore探索 | →Planning |
| [Planning](phases/phase-planning.md) | 设计+确认计划 | MECE分解+DAG+Agent分配+用户确认 | 同意→Execution / 拒绝→PromptOptimization / 取消→Cleanup |
| [Execution](phases/phase-execution.md) | 执行任务 | 智能并行(≤2)+冲突检测+HITL | →Verification |
| [Verification](phases/phase-verification.md) | 验证结果 | 验收检查（passed/failed） | passed→QualityGate / failed→Adjustment |
| QualityGate | 质量评估（非失败判定） | 质量分 vs 迭代阈值 | 达标→Cleanup / 不达标→PromptOptimization |
| [Adjustment](phases/phase-adjustment.md) | 失败调整（条件触发） | 4级升级(retry→debug→replan→ask_user) | 全部→PromptOptimization（ask_user先获取用户方向） |
| [Cleanup](phases/phase-cleanup.md) | 清理 | 删除计划+清理检查点+保存记忆 | →End |
...
flowchart TD
    ...
    Init --> PO[PromptOptimization: 提示词优化]
    PO --> DeepRes{需要研究?}
    PO --> UserChoice{用户选择?}
    ...
    Verify --> Decision{结果?}
    Decision -->|passed| QG{质量达标?}
    Decision -->|failed| Adjust[Adjustment: 失败调整]
    QG -->|达标| Clean[Cleanup: 清理]
    QG -->|不达标| PromptOpt
