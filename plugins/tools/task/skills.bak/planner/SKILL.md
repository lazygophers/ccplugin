...
AskUserQuestion({
  "questions": [{
    "question": "计划已生成（${task_count}个任务，.lazygophers/tasks/${task_id}/plan.md），是否批准执行？",
...
1. **更新 metadata.json**：读取 `.lazygophers/tasks/{task_id}/metadata.json`，更新 `result` 字段和 `updated_at`
   ```json
   {
     "result": {
...
planner 写入的文件：
- `.lazygophers/tasks/{task_id}/plan.md` — 完整计划文档
- `.lazygophers/tasks/{task_id}/tasks.json` — 任务数组
- `.lazygophers/tasks/{task_id}/metadata.json` — 更新 `result` 字段（status/report/task_count/user_feedback）
