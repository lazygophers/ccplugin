...
- **🔍 项目探索体系**：8 个专业 Explorer Agents（通用/代码/前端/后端/数据库/API/测试/基础设施），基于 AST+PageRank 代码分析（2024-2025 最新研究）
...
| **数据库分析** | `task:explorer-database` | Schema、关系、索引、迁移 | 数据库架构和 ORM 模型分析 |
| **API 分析** | `task:explorer-api` | 端点、参数、认证 | REST/GraphQL/gRPC API 分析 |
| **测试分析** | `task:explorer-test` | 框架、覆盖率、质量 | 测试体系评估和缺口识别 |
| **基础设施** | `task:explorer-infrastructure` | Docker、CI/CD、云服务 | DevOps 配置和部署分析 |
| **依赖分析** | `task:explorer-dependencies` | 依赖树、安全、许可证 | 依赖审计和供应链分析 |
| **任务规划** | `task:planner` | 计划设计、任务分解 | 复杂任务 MECE 分解 |
| **任务执行** | `task:execute` skill | 并行编排、进度跟踪 | 多任务调度、依赖管理 |
...
│       → 继承 code 能力 + Schema/关系/索引分析
│
├── API 设计分析
│   └── explorer-api（API 探索）
│       → 继承 code 能力 + REST/GraphQL/gRPC 分析
│
├── 测试体系分析
│   └── explorer-test（测试探索）
...
Task 插件提供了完整的项目探索体系，通过 9 个专业 Explorer Agents 实现从宏观到微观的渐进式理解。

**Explorer Agents 4 层架构**：
...
领域层（Domain，继承 code 能力）：
    explorer-code
        ├─→ explorer-frontend（前端特化）
        ├─→ explorer-backend（后端特化）
        ├─→ explorer-database（数据库特化）
        ├─→ explorer-api（API 特化）
        └─→ explorer-test（测试特化）

横切层（Cross-cutting，独立分析）：
    ├─→ explorer-infrastructure（基础设施）
    └─→ explorer-dependencies（依赖分析）
```
...
- **渐进式理解** - 从 general（宏观）→ code/frontend/backend/database/api/test（微观）
- **能力继承** - 领域层 5 个 explorer 继承 code 的符号索引和依赖分析能力
- **结构化输出** - 所有 explorer agents 输出标准 JSON 格式，便于后续处理
- **智能路径选择** - 根据 project_type 和分析需求自动选择合适的 explorer
- **横切关注点** - infrastructure/dependencies 独立于代码结构，专注配置和供应链分析
...
Task 插件提供 14 个 Agents，按职责分为 3 类：任务编排（2 个）、项目探索（8 个）、辅助工具（4 个）。

### 任务编排 Agents

| Agent | 职责 | 模型 | 内存 | 输出格式 | 适用场景 |
|-------|------|------|------|---------|---------|
| `task:planner` | 计划设计与任务分解 | opus | project | JSON | 复杂任务规划、MECE 分解、依赖建模 |
...
| `task:explorer-database` | 数据库架构探索 | sonnet | project | JSON | ← explorer-code | Schema、表关系、索引、迁移 |
| `task:explorer-api` | API 设计探索 | sonnet | project | JSON | ← explorer-code | REST/GraphQL/gRPC 端点分析 |
| `task:explorer-test` | 测试体系探索 | sonnet | project | JSON | ← explorer-code | 框架、覆盖率、质量评估 |
| `task:explorer-infrastructure` | 基础设施探索 | sonnet | project | JSON | - | Docker/K8s、CI/CD、云服务 |
| `task:explorer-dependencies` | 依赖分析探索 | sonnet | project | JSON | - | 依赖树、安全审计、许可证 |
...
**核心职责**：
- 编排计划设计流程（调用 planner，planner 自动格式化并写入计划文件）
- 智能路径选择（首次/用户重新设计需用户确认，自动重规划跳过）
- 处理用户反馈和计划调整
...
if iteration > 1 and replan_trigger in ["adjuster", "verifier"]:
    # 自动重规划
    planner_result = Skill(skill="task:planner", ...)  # 含格式化+写文件
    # 自动批准，plan_md_path 在 planner_result 中
else:
    # 首次或用户重新设计
    planner_result = Skill(skill="task:planner", ...)  # 含格式化+写文件
    AskUserQuestion(...)  # 请求用户批准
```
...
| `task:explorer-database` | 数据库架构探索 | JSON | serena + Glob/Grep | PostgreSQL/MySQL/MongoDB | Schema、表关系、索引、迁移 |
| `task:explorer-api` | API 设计探索 | JSON | serena + Glob/Grep | REST/GraphQL/gRPC | 端点分析、参数、认证机制 |
| `task:explorer-test` | 测试体系探索 | JSON | Glob/Grep + Bash | Jest/pytest/Go/JUnit | 框架、覆盖率、质量评估 |
| `task:explorer-infrastructure` | 基础设施探索 | JSON | Glob/Grep + Read | Docker/K8s/CI-CD | 容器化、部署、云服务 |
| `task:explorer-dependencies` | 依赖分析探索 | JSON | Glob/Grep + Bash | npm/pip/go mod/cargo | 依赖树、安全审计、许可证 |
...
**核心原则**：
- Schema 是数据库骨架，ORM 模型优先
- 关系追踪（一对一、一对多、多对多）
- 索引评估覆盖常用查询模式
- 迁移历史记录演进过程

**ORM 检测**：
- Prisma: `schema.prisma` 中 `model User { ... }`
- GORM: `type User struct { gorm.Model ... }`
- SQLAlchemy: `class User(Base): __tablename__ = "users"`
- TypeORM: `@Entity() class User { @Column() ... }`
- Django: `class User(models.Model): ...`
- Drizzle: `export const users = pgTable('users', { ... })`

**文档**：[skills/explorer-database/SKILL.md](skills/explorer-database/SKILL.md)

#### task:explorer-api

**核心原则**：
- 规范文件优先（OpenAPI/Swagger/GraphQL schema）
- 端点完整性（包括未文档化的隐藏端点）
- 契约驱动（请求参数、响应格式、错误处理）
- 安全维度（认证方式、授权策略）
...
**文档**：[skills/explorer-api/SKILL.md](skills/explorer-api/SKILL.md)

#### task:explorer-test

**核心原则**：
- 数据驱动评估（覆盖率、测试数量、断言模式）
- 测试金字塔（70% 单元 / 20% 集成 / 10% E2E）
- 质量重于数量（断言质量、边界覆盖）
- 精准缺口识别（按风险排序）

**框架检测**：
- Jest/Vitest: `**/*.test.{ts,js}`、`**/*.spec.{ts,js}`
- pytest: `**/test_*.py`、`**/*_test.py`
- Go testing: `**/*_test.go`
- JUnit: `**/Test*.java`、`**/*Test.java`

**文档**：[skills/explorer-test/SKILL.md](skills/explorer-test/SKILL.md)
...
- 配置文件是真相来源
- 安全敏感（不暴露 .env 实际值）
- 环境差异（dev/staging/production）
- 依赖拓扑（应用→数据库→存储）

**检测范围**：
- 容器: Docker, Docker Compose, Podman
- 编排: Kubernetes, ECS, Cloud Run
- CI/CD: GitHub Actions, GitLab CI, CircleCI, Jenkins
- 云服务: AWS, GCP, Azure, Vercel, Netlify
- IaC: Terraform, CloudFormation, Pulumi

**文档**：[skills/explorer-infrastructure/SKILL.md](skills/explorer-infrastructure/SKILL.md)
...
**文档**：[skills/explorer-dependencies/SKILL.md](skills/explorer-dependencies/SKILL.md)
...
│   ├── explorer-database.md     # 数据库探索
│   ├── explorer-api.md          # API 探索
│   ├── explorer-test.md         # 测试探索
│   ├── explorer-infrastructure.md # 基础设施探索
│   └── explorer-dependencies.md # 依赖探索
├── skills/
│   ├── loop/                    # 循环控制规范
│   │   ├── SKILL.md
...
│   ├── explorer-database/       # 数据库探索规范
│   ├── explorer-api/            # API 探索规范
│   ├── explorer-test/           # 测试探索规范
│   ├── explorer-infrastructure/ # 基础设施探索规范
│   ├── explorer-dependencies/   # 依赖探索规范
...
- **Task 内置 agents** - 仅用于流程编排（planner/execute/verifier/adjuster/finalizer/prompt-optimizer/explorer-*）
