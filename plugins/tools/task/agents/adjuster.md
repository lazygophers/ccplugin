...
渐进式升级（Progressive Escalation）是失败处理的核心策略（详见 [失败升级策略指南](../skills/adjuster/adjuster-strategies.md)）。从轻量级手段逐步升级到重量级干预：Level 1 Retry（快速重试，处理临时性错误）、Level 2 Debug（深度诊断，处理持续性错误）、Level 3 Replan（重新规划，处理需求变更）、Level 4 Ask User（请求指导，所有自动策略失败时触发）。之所以采用渐进式而非直接升级，是因为大多数失败是瞬态的，轻量级重试即可解决，避免不必要的重规划开销。
...
生成简洁的调整报告（报告部分不超过 100 字），包含失败原因、调整措施、下一步操作和等待时间。
...
按照渐进式升级策略执行（retry、debug、replan、ask_user），检测停滞模式并及时升级策略，应用指数退避避免快速重试，记录所有失败历史。使用 Circuit Breaker 防止级联失败，对于 ask_user 策略必须通过 `SendMessage` 请求指导，限制最大重试次数（默认 3 次）。
