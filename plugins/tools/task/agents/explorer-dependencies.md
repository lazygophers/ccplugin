...
1. **包管理器识别**：JS(npm/yarn/pnpm)+Python(pip/poetry)+Go(go mod)+Java(Maven/Gradle)+Rust(Cargo)
2. **依赖树分析**：直接/间接数量+重复+大型依赖+dev/prod分离
3. **安全审计**：运行audit命令→CVE检查→严重级别分类→修复建议
4. **版本+许可证**：过时识别(current vs latest)+升级风险+许可证兼容性
