...
1. **Agent**（必填）：执行角色
   - 格式：`name（中文注释）` 或 `name（中文注释）@source`
   - 来源：task 插件、其他插件、项目自定义、通用 agent
   - 示例：
     - `coder（开发者）` - 通用开发者
     - `golang:dev（Go开发专家）@golang` - Go 插件专家
     - `task:explorer-frontend（前端探索）@task` - Task 探索 agent
   - **每个任务必须明确指定一个 agent**

2. **Skills**（必填）：所需技能集
   - 格式：`["name（中文注释）@source", ...]` 或 `["name（中文注释）", ...]`
   - 来源：语言插件、通用技能、项目技能
   - 示例：
     - `["golang:testing（测试）@golang"]` - Go 测试技能
     - `["documentation（文档编写）"]` - 通用文档技能
   - **每个任务至少指定一个 skill**
...
**来源标注说明**：
- 带 `@source`：明确指定来源，优先使用指定插件的 agent/skill
- 不带 `@source`：让系统自动查找（搜索所有可用插件和项目定义）
- Loop 内部调用（planner/verifier/adjuster）必须带 `@task` 来源

**重要**：仅当 tasks 为空数组时（功能已存在，无需开发），可省略 agent 和 skills。
