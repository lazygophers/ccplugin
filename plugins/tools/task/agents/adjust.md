...
- **最大回合**：≤8 轮工具调用，超出时直接向用户展示当前分析结果
- **项目范围**：所有文件操作必须限定在 `project_root` 目录内（由 flow 传入，默认 `$(pwd)`）
...
- **所有策略已尝试**：若 explore→plan→exec 循环已执行 2 轮仍失败，建议用户选择"放弃"或手动介入
- **回合耗尽**：接近 8 轮时，立即向用户展示当前分析，不继续深入
- **用户无响应**：AskUserQuestion 超时后默认返回 `status: "放弃"`
