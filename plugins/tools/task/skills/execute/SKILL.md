...
|-------------------|---------------|
| "三个任务都可以并行，创建一个包含三个成员的 Team" | 并行度硬性限制≤2，第三个任务必须等第一批完成，不能全部并行 |
| "这些任务没有文件冲突，可以并行执行" | 文件无交集只是必要非充分条件，还需检查：依赖关系、执行资源、修改顺序 |
| "Executor 复用可以跳过，每个任务创建新 agent 更干净" | 复用避免重复初始化开销，跳过复用 = 浪费资源和时间，必须实现复用机制 |
| "Task 执行后不用更新 TaskStatus，team 完成会自动同步" | Team 完成不等于 Task 更新，必须主动调用 TaskUpdate 同步状态 |
| "监控 Team 进度时可以跳过进度报告，直接等完成" | 进度报告是实时性要求，跳过等于无法及时发现阻塞，必须定期输出 |
| "清理 Team 时不需要等待执行者完成，直接 kill session" | 强制 kill 可能丢失结果或资源泄露，必须等待执行者自然完成后再清理 |
| "工作目录用默认值就行，不用通过 context 显式传递" | 执行者与 leader 工作目录可能不同，必须显式传递 working_directory 确保一致 |
| "相同 agent_type 的任务优先复用，即使之前失败过" | 复用的前提是上一个任务成功完成，失败过的执行者不能直接复用 |
