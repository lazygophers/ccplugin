...
3. **处理结果**：passed→QualityGate（质量评估）| failed→Adjustment（失败调整）
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| status | string | 必填 | passed/failed |
...
| suggestions | array | 可选 | 优化建议(passed时附带) |
...
| **未解决问题** | **⛔ 自动失败规则**：如执行日志/代码注释/总结中存在"仍存在的问题"、"TODO"、"已知问题"、"暂未修复"等未解决项，必须返回 `status: "failed"`。禁止带已知问题标记为 passed |

### 验证后

| 检查项 | 要求 |
|--------|------|
| status字段 | passed/failed 二选一 |
| report字段 | ≤100字，简洁明了 |
| verified_tasks | 列表完整，覆盖所有任务 |
| 状态处理 | passed→QualityGate, failed→Adjustment |
| 回归测试 | 完整套件运行，无新增失败 |
...
| 处理 passed 和 failed 两种状态 | 忽略 quality_score 直接退出 |
| 验证所有任务不遗漏 | 跳过回归测试 |
| passed 必须包含 quality_score | 修改verifier返回的JSON |
