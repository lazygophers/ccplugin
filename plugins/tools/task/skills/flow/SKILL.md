...
3. **禁止未经用户确认就进入 plan** — align 阶段必须通过 AskUserQuestion 获得用户明确确认，这是硬性门控
...
### 步骤 2：align（范围对齐）— 必须获得用户确认

1. 执行 `task update {task_id} --status=align`
2. 调用 `Skill("task:align")`，传入 task_id
3. **关键检查**：align 必须通过 AskUserQuestion 获得用户明确确认。验证 align.json 已写入且包含 `user_confirmed: true`
4. 如果 align 返回 need_explore=true → 转步骤 2a
5. 如果 align.json 不存在或 user_confirmed != true → **禁止进入 plan**，重新调用 align
6. 用户确认后 → 转步骤 2b（快车道判断）
