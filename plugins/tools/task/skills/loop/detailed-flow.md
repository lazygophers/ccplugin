...
| [PromptOptimization](phases/phase-prompt-optimization.md) | 优化用户输入（可选） | 质量评估+5W1H+WebSearch+用户选择 | A/B→DeepResearch或Planning / C→循环 |
| [DeepResearch](phases/phase-deep-research.md) | 深入研究（可选） | 复杂度评估+Explore探索 | →Planning |
| [Planning](phases/phase-planning.md) | 设计+确认计划 | MECE分解+DAG+Agent分配+用户确认 | 自动批准→Execution / 用户批准→Execution/拒绝→Planning |
| [Execution](phases/phase-execution.md) | 执行任务 | 智能并行(≤2)+冲突检测+HITL | →Verification |
| [Verification](phases/phase-verification.md) | 验证结果 | 验收检查+质量评分 | passed→Finalization / suggestions→Planning / failed→Adjustment |
| [Adjustment](phases/phase-adjustment.md) | 失败调整（条件触发） | 5级升级(retry→debug→replan→ask_user→escalate) | retry/debug→Execution / replan→Planning / ask_user→用户 |
