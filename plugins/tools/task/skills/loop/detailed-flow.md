...
| [Planning](phases/phase-planning.md) | 设计+确认计划 | MECE分解+DAG+Agent分配+用户确认 | 同意→Execution / 拒绝→PromptCheck / 取消→Cleanup |
...
| QualityGate | 质量评估（非失败判定） | 质量分 vs 迭代阈值 | 达标→Cleanup / 不达标→PromptCheck |
