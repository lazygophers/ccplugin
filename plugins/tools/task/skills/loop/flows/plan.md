...
### PromptOptimization：提示词优化（评分<9触发）

调用 `task:prompt-optimizer` skill 评估质量(清晰度/完整性/可执行性)。≥9分跳过，<9分识别缺失5W1H并提问获取信息，<6分触发WebSearch。优化完成后展示原始提示词 vs 优化后提示词对比，由用户选择：A(使用原始)、B(使用优化)、C(重新优化)。根据用户选择更新user_task。
...
| `rejected` | 提取 user_feedback → replan_trigger="user" → 回到 PromptCheck（评估提示词质量） |
| `no_tasks` | 进入完成阶段 |
| `cancelled` | 进入中止（Terminated） |
...
3. 回到 PromptCheck（评估提示词质量，评分<9则优化，≥9则进入复杂度评估→Planning）
4. **循环直到 planner 返回 `confirmed` 或 `cancelled`**

## 状态转换

- confirmed → Execution(任务执行)
- rejected → PromptCheck（重新评估提示词质量）
- no_tasks → Finalization(完成)
- cancelled → Terminated(中止)
