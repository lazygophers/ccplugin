...
4. if auto_approve=true: 自动批准并进入任务执行；else: 调用 `AskUserQuestion({"questions": [{"question": "...", "header": "计划确认", "options": [{"label":"批准执行","description":"..."},{"label":"修改计划","description":"..."},{"label":"取消任务","description":"..."}], "multiSelect": false}]})` 请求用户批准（按批准判定规则处理）
...
3. 处理 planner 返回结果：
   - 有 questions → AskUserQuestion 询问用户（参数格式：`{"questions": [{"question":"...","header":"...","options":[...],"multiSelect":false}]}`）
   - tasks 为空 → skip_execution
   - tasks 非空 → 提取 `plan_md_path`，更新 context
4. 调用 `AskUserQuestion({"questions": [{"question": "...", "header": "计划确认", "options": [{"label":"批准执行","description":"..."},{"label":"修改计划","description":"..."},{"label":"取消任务","description":"..."}], "multiSelect": false}]})` 展示计划摘要，请求用户批准（按批准判定规则处理）
