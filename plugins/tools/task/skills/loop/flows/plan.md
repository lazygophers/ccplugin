...
首次迭代必须执行，后续迭代仅在用户提供新输入时触发（增量修订，非重写），无新输入则跳过。流程：TaskDecomposition（任务分解+边界定义）→ ClarificationDialog（澄清对话，最多3轮）→ SpecGeneration（生成规格说明）→ PromptPersistence（写入prompt.md）→ UserConfirmation（用户选择A/B/C）。prompt.md 的验收标准是迭代是否继续的核心依据。
