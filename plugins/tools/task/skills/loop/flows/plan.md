...
| `rejected` | 提取 user_feedback → replan_trigger="user" → 回到 PromptOptimization（评估提示词质量） |
| `no_tasks` | 进入完成阶段 |
| `cancelled` | 进入完成阶段 |
...
3. 回到 PromptOptimization（评估提示词质量，评分<9则优化，≥9则进入复杂度评估→Planning）
...
- no_tasks / cancelled → Finalization(完成)
