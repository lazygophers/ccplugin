...
调用 `task:prompt-optimizer` skill，每次迭代必须执行。流程：BoundaryAnalysis（任务边界分析）→ StructuredQuestioning（5W1H提问）→ PromptGeneration（生成优化提示词）→ PromptPersistence（写入prompt.md）→ UserConfirmation（用户选择A/B/C）。根据用户选择更新user_task。
