...
首次迭代必须执行，后续迭代仅在用户提供新输入时触发（增量修订已有 prompt.md，非重写），无新输入则跳过。调用 `Skill(skill="task:prompt-optimizer")` 将任务描述转化为可执行规格说明，写入 `.claude/tasks/{task_id}/prompt.md`。
...
| **B: 使用原始提示词** | 保持用户原始输入 | 将原始版本写入 prompt.md → 进入复杂度评估 |
| **C: 重新优化** | 用户提供反馈 | 收集反馈 → 重新调用 prompt-optimizer |
...
**整个阶段只需一次 Skill() 调用**。planner 内部完成：设计计划 → 写入文件 → 用户确认（或自动批准）→ 更新 metadata.json 的 `result` 字段。
...
【强制】调用 task:verifier skill 验证。调用时必须传递完整上下文字段（project_path、task_id、iteration、plan_md_path、working_directory、user_task），确保 verifier 能独立定位项目和计划文件。verifier 将结果写入 metadata.json 的 `result` 字段。loop 读取 `result.status` 分支**必须立即继续**：
...
调用 task:adjuster skill 分析。adjuster 将结果写入 metadata.json 的 `result` 字段。loop 读取 `result.strategy` 分支**必须立即继续**：
