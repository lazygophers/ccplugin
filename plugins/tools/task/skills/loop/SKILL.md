...
`iteration += 1`，更新 metadata.json：`phase → "planning"`, `status → "planning"`, `updated_at → 当前时间`
...
**前置条件**：计划文件存在，已获得用户批准。更新 metadata.json：`phase → "execution"`, `status → "executing"`, `updated_at → 当前时间`
...
**前置条件**：所有任务已执行完成。更新 metadata.json：`phase → "verification"`, `status → "verifying"`, `updated_at → 当前时间`
...
更新 metadata.json：`phase → "adjustment"`, `status → "adjusting"`, `updated_at → 当前时间`
...
**前置条件**：质量评估通过 / no_tasks / cancelled。更新 metadata.json：`phase → "cleanup"`, `status → "completed"`, `updated_at → 当前时间`
