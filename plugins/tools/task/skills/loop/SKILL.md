...
- 质量递进：根据任务复杂度动态调整（简单 1轮，中等 2轮，复杂 3轮，非常复杂 4轮）
- 最小迭代次数：动态确定（基于复杂度评估）
...
| 任务复杂度 | 最小迭代 | 说明 |
|-----------|---------|------|
| Simple（0-30分） | 1 轮 | 简单任务，快速交付 |
| Moderate（31-60分） | 2 轮 | 中等任务，两轮优化 |
| Complex（61-100分） | 3 轮 | 复杂任务，三轮递进 |

**质量阈值递进**（无最大轮数限制）：
| 迭代轮次 | 质量阈值 | 等级 |
|---------|---------|------|
| 第 1 轮 | 60分 | Foundation（基础） |
| 第 2 轮 | 75分 | Enhancement（增强） |
| 第 3 轮 | 85分 | Refinement（精化） |
| 第 4+ 轮 | 90分 | Excellence（卓越，保持） |

**复杂度评估**：基于文件数、技术栈、任务类型、质量要求（总分 100）
**终止条件**：质量达标 + 最小迭代 + 用户满意（可无限迭代直到完美）
