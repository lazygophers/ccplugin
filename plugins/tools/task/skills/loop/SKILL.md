...
1. **铁律遵守**：是否通过 Agent() 调用而非直接工具？是否遗漏必要步骤（Planner三层学习/Verifier/Finalizer）？
...
**子 agent**：task:prompt-optimizer、task:planner（含格式化+写文件）、task:verifier、task:adjuster、task:finalizer
...
1. 【强制】调用 `Agent(subagent_type="task:finalizer", prompt="...")` （删除计划文件、清理检查点、清理任务状态文件、停止运行中任务）。调用时必须传递完整上下文字段（project_path、task_id、iteration、plan_md_path、working_directory、user_task），确保 finalizer 能独立定位需要清理的资源。
...
4. **Finalizer 调用检测**：
   ...
   - 检测时机：准备输出最终完成消息前
   - 检测方法：确认已调用 `Agent(subagent_type="task:finalizer", ...)` 并获得返回结果
