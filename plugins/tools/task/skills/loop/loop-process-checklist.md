...
| **任务状态文件更新** | **`.claude/tasks/{task_id}/status.json` status 为 completed/failed（必须在 finalizer 之前）** | **必须** |
...
| **成本预算未超限** | **验证未触发熔断（Circuit Break），或触发后正确进入 Finalization** | **必须** |
| **残留计划文件已清理** | **Initialization 初始化时检测并清理前次遗留的 .claude/plans/*.md** | **必须** |
