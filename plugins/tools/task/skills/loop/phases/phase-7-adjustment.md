...
失败调整阶段：分析失败原因，决定恢复策略，支持5级渐进升级和HITL审批。
...
| Level | 策略 | 触发条件 | 退避 | 下一步 |
|-------|------|---------|------|--------|
| 1 | retry | 首次失败/临时错误 | 0s | Phase 5 |
| 2 | debug | 重试失败/逻辑错误 | 2s | Phase 5 |
| 3 | replan | 调试失败/计划缺陷 | 4s | Phase 4(自动批准) |
| 4 | ask_user | 重规划失败/停滞 | - | 用户决定 |
| 5 | escalate | 超出能力/用户请求 | - | 高级agent |
