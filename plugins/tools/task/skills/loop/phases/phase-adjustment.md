...
| `ask_user` | - | 用户指导 → PromptOptimization | **必须立即**请求用户指导，获得响应后回到提示词评估 |

### ask_user 处理流程

当 `result.strategy` 为 `ask_user` 时，loop 必须通过 `AskUserQuestion` 请求用户指导：

```json
AskUserQuestion({
  "questions": [{
    "question": "${result.question}",
    "header": "[MindFlow·${task_id}·失败调整]",
    "options": [
      {"label": "提供修改建议", "description": "描述如何修复或调整任务"},
      {"label": "跳过该任务", "description": "标记为失败并继续其他任务"},
      {"label": "取消整个任务", "description": "放弃当前任务"}
    ],
    "multiSelect": false
  }]
})
...
用户响应后，将反馈内容更新到 `context.user_feedback`，然后回到 PromptOptimization 阶段。
