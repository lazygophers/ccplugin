...
Agent(subagent_type="task:planner", prompt="
  ...
  task_id: {task_id}
  iteration: {iteration}
  plan_md_path: .lazygophers/tasks/{task_id}/plan.md
...
**索引更新**：Planning 完成后，更新 `.lazygophers/tasks/index.json` 中对应任务的 `phase` 和 `updated_at`。**禁止使用 Write/Edit 工具，必须使用 Bash 工具执行以下 jq 命令**。
