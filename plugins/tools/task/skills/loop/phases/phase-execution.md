...
| 维度 | 权重 | 低 | 高 |
|------|------|---|---|
| 文件数 | 30% | 1-2个(0-15) | 5+个(60-100) |
| 依赖深度 | 25% | 0-1层(0-20) | 3+层(70-100) |
| 预估token | 20% | <10K(0-20) | >50K(100) |
| 文件冲突 | 25% | 无共享(0) | 写入共享(100,禁止并行) |

等级：<30 低 | 30-70 中 | >70 高
...
获取就绪任务 → 评估复杂度 → 检测文件冲突(构建 file_map) → 计算并行度 → 选择无冲突批次 → 执行
