...
| 状态 | 条件 | 质量分 | 下一步 | 强制要求 |
|------|------|--------|--------|---------|
| passed | 全部验收通过，无建议 | ≥80 | Phase 8（完成） | **必须立即**进入 Finalizer 清理 |
| suggestions | 全部通过，有优化空间 | 60-79 | Phase 4（自动重规划） | **必须立即**回到计划设计（`replan_trigger="verifier"`） |
| failed | 至少一项验收未通过 | <60 | Phase 7（失败调整） | **必须立即**进入失败调整 |
