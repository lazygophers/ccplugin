...
| 级别 | 策略 | 触发条件 | 操作 | 下一步 |
|------|------|---------|------|--------|
| 1 | Retry | 首次失败/临时错误 | 调整参数+指数退避(delay=base×2^n, max 60s) | Phase 5 |
| 1.5 | Self-Healing | 匹配17类可预测错误 | 自动修复(详见self-healing.md) | Phase 5 |
| 2 | Debug | Retry×3失败/Self-Heal×2失败 | 收集日志+检查依赖配置+诊断报告 | Phase 5 |
| 2.5 | Micro-Replan | Debug×3无效 | 仅重规划失败任务+直接依赖，保留成功任务 | Phase 4 |
| 3 | Full Replan | Micro-Replan失败/需求变更 | 调用planner重新设计+保留已完成工作 | Phase 4 |
