...
1. **交付物定义**：最终应产出什么（代码/配置/文档），用具体动词描述
2. **任务边界**：in-scope（本次要做的）/ out-of-scope（明确不做的）
3. **影响范围**：涉及的模块、文件、功能区域
4. **验收标准**：可量化、可独立验证的完成条件（每条一个判定点）
...
基于分解结果和用户回答，生成结构化的规格说明（≤500字）。这就是写入 prompt.md 的内容。
...
- `boundary`：Verification 检查交付物是否超出 in-scope 或遗漏，是否引入 out-of-scope 变更
