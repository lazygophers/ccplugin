...
| 指标数据 | `.claude/tasks/{task_id}/metrics.json` | 可观测性指标 | 始终删除 |
...
- **文件**：扫描以下目录
  - `.claude/tasks/{task_id}/` — 计划文件(plan.md)、审批日志、指标数据、元数据、任务清单
...
4. **指标数据**：删除 `.claude/tasks/{task_id}/metrics.json`
5. **计划文件**：删除 `.claude/tasks/{task_id}/plan.md`（包括所有状态的文件）
