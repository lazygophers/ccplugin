...
读取顺序：CLAUDE.md(必读) → .claude/rules/(按需) → .claude/memory/(按需) → README.md(推荐)

优先级：rules(具体) > CLAUDE.md(项目级) > memory(历史积累)。发现冲突时 SendMessage 提醒用户。

## Spec-Driven Planning

规划前生成四维规范：功能(What) | 技术(How) | 质量(Quality) | 合规(Compliance)。确保规范符合项目风格、架构约束、复用现有组件、与 CLAUDE.md/.claude/rules/ 一致。
