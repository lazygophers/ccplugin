...
- `status: "rejected"` → 提取 user_feedback，重新调用 planner
...
JSON 输出：`{status, plan_md_path?, report(≤200字), tasks[], dependencies{}, parallel_groups[[]], iteration_goal, acceptance_criteria[], user_feedback?, task_count?}`

- `status`: "confirmed"（用户批准/自动批准）| "rejected"（用户要求修改）| "no_tasks"（无需执行）| "cancelled"（用户取消）
- `plan_md_path`: 已写入的计划文件绝对路径（confirmed/rejected 时必填）
...
**重要**：tasks 非空时，planner 必须完成阶段5（写文件）和阶段6（用户确认），然后返回最终 status
