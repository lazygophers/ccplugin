...
### OutputJSON：返回状态 JSON（强制，禁止跳过）

**⚠️ 防中止规则（最高优先级）**：

UserConfirmation 完成（用户确认/自动批准/取消）后，**绝对禁止**在此处结束回复。必须**立即**在同一回复中完成以下操作：

1. **构建状态 JSON**：包含 `status`、`report`、`task_count`、`user_feedback`（rejected 时）等状态字段
2. **返回 JSON**：将状态 JSON 作为返回值，供 loop 主体解析
3. **禁止**：输出"计划已确认"等摘要后就停止。**必须**返回包含 status 字段的 JSON 后才能结束 planner skill 的执行
4. **禁止**：将计划内容（tasks/dependencies/parallel_groups 等）打印到对话中——这些已写入 plan.md 和 tasks.json 文件

**自检**：在准备结束回复前，检查"我是否已返回了包含 status 字段的 JSON？"——如果没有，**必须继续**。
...
JSON 返回值（仅状态，计划详情已写入文件）：`{status, report(≤200字), task_count?, user_feedback?}`
...
**计划详情存储在文件中**（不在 JSON 返回值中重复）：
- `.claude/tasks/{task_id}/plan.md` — 完整计划文档
- `.claude/tasks/{task_id}/tasks.json` — 任务数组（含 id/description/agent/skills/files/acceptance_criteria/dependencies）
