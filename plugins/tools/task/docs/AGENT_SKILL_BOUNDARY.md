...
| **生命周期** | 运行时 | 编译时 |
| **调用方式** | `Agent(agent=...)` | `Skills(...)` 或被引用 |

### 8.2 重叠问题总结

- **prompt-optimizer**: Skill 包含过多执行逻辑 → 需要重构
- **execute**: 缺少对应 Agent → 需要创建 agent 或简化 Skill

### 8.3 优化收益

- ✅ 职责单一：Agent 专注执行，Skill 专注规范
- ✅ 易于维护：修改执行逻辑只需改 Agent，修改规范只需改 Skill
- ✅ 易于理解：新开发者快速区分"怎么做"和"做什么"
- ✅ 减少冗余：消除重复描述，减少文档大小
- ✅ 提升质量：统一标准后更容易发现不一致
