请在开始任务之前，通过采访的方式来提了解我的想法和意图。采访时请注意：
...
**优先级**: Skills > 个人习惯 > 语言标准 | Skills 与本文冲突时优先 Skills
**时机**: 编码前读 → 编码中对照 → 提交前审查
**违规**: 立即修正，不得提交

## Agent 选择策略

- **优先级**: 语言+任务专用 → 语言通用 → 任务通用 → 完全通用
- **独立任务并行**:
- **分阶段串行**
- **混合模式**（复杂项目使用）:

```
并行组1: 探索分析类 (explore + debug)
并行组2: 质量检查类 (code-review + security-auditor)
并行组3: 测试开发类 (golang-test + react-test)
串行依赖: architect → 并行组1,2,3 → 最终集成
```
