...
def handle_hook() -> None:
    ...
    try:
        ...
        logging.info(f"[fasthttp] 接收到事件: {event_name}")
    ...
    if go_mod_path.exists():
        try:
            with open(go_mod_path, 'r', encoding='utf-8') as f:
                content = f.read()
                if 'github.com/valyala/fasthttp' in content:
                    logging.info("[fasthttp] 检测到 fasthttp 项目")
        except Exception as e:
            logging.debug(f"[fasthttp] 读取 go.mod 失败: {e}")
