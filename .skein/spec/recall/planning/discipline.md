...
- MUST：三环节同构硬门·STOP，统一格式 `🛑 <级>: 未 <状态命令> 禁 <动作> (硬门·STOP) ... 违反 → 回退先 <状态命令>`：
  - **task 级**：未 `skein confirm --approved`（已吸收原 `start` 全部职责）禁进 exec (待处理/调研中态 task 禁派 subtask、禁跑 exec)
  - **subtask 级**：未 `skein claim` / `skein subtask start` 占 `pools.work` 槽禁派 agent (pending/failed 态 subtask 禁直接派)
...
| 禁 | 改为 |
|---|---|
| main 待处理/调研中态 task 直接派 subtask | 先 `skein confirm --approved` 进进行中再派 (吸收原 start) |
