...
**取舍 5 · 中文同义触发词照删（实测已定，s1 出数据）** — 原做法是拿 `git-commit` 当实验组实测定夺。s1 已跑完：
...
6/6 → 6/6，**触发率未下降**。结论：**其余 12 份的同义触发词列表一律照删，不回填**，description 只留一 branch 一 trigger。下游 subtask 直接照此执行，不再各自重跑实验。
