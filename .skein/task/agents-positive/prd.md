...
## 验收标准
可执行、可核对的完成断言 (逐条):
- [ ] 9 个文件中的否定式表述 (禁/不得/不要/勿/不准/不该) 全部转为正向, 或列入豁免清单并说明理由。
- [ ] 逐条比对转写前后语义等价: 每条给出「原文 → 新文」对照, 边界与 why 无丢失。
- [ ] 跨文件重复的同类约束措辞统一 (公共铁律、工具失败标记、缺信息回传 三类至少各收敛为一种措辞)。
- [ ] 工作流步骤、章节结构、frontmatter 未被改动 (diff 只落在正文措辞)。
