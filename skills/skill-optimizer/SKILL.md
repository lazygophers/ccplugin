...
when_to_use: 用户说「优化/改改这个 skill」「skill 不触发/误触发」「agent.md 优化」「提升 skill 质量」「skill review」「回归」时；skill 触发不准/失败模式缺失/质量退化时自动调用。深度自动评分路由 darwin。
---
...
1. **validation-gated**：任何编辑必须经 held-out 测试前后对比，**严格提升才保留**，否则回滚。禁凭「我觉得改完更好」直接落地（SkillLens 实证 LLM-as-judge 准确率仅 46.4%，接近随机）。
...
1. **回归测试**：改完跑原 skill 的 eval 场景（若 skill-author Phase 5 evals.json 存在则复用），确认未 break 已有能力。
2. **变更语义标注**：
