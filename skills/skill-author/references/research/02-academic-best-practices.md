...
> ⚠️ **诚实标注**：darwin-skill 引用的 SkillLens (arXiv 2605.23899) 与 SkillOpt (arXiv 2605.23904) 经独立检索（多引擎 web_search）**无法核实存在**，疑似虚构 arXiv ID。本维度仅采纳有独立佐证的方法论，darwin-skill 自身作为本地一手实证（已开源自查）。
...
darwin-skill 引用的具体论文数字（46.4%→73.8% 等）因论文无法核实，**不纳入产物**。但其方法论内核可独立成立：
