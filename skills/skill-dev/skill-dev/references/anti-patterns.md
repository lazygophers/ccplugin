...
| 16 | 同一规则跨段/跨 skill 复述 3+ 次（Iron Law + ❌反例 + 自欺表 + 流程各述一遍） | 单一真值源：规则在**流程/canonical 段**述 1 次，其余段**指针引用**（「走 step N」）；复述 → 措辞漂移 → 逻辑分叉 |
| 17 | 负强化三段并列（🔒Iron Law + ❌反例 run-on + 自欺兜底表 全否定措辞） | 合并为 1 张 **✅正向配方表**（`场景 → 正确做法 (❌反面)`）；正向指令比否定强，禁语降为括注。范式见 skein-check |

### P2 中等（质量损耗）

| # | 反模式 | 正例 |
|---|--------|------|
| 18 | 术语混用（field/box/element） | 选定一个贯穿 |
| 19 | 时间敏感信息内联 | 放 old patterns `<details>` |
| 20 | 脚本 punt 错误给 Claude | 显式 try/except + 默认值 |
| 21 | voodoo constants（TIMEOUT=47） | 注释为何这个值 |
| 22 | 假设包已安装 | 显式 `pip install` |

### P3 结构性

| # | 反模式 | 正例 |
|---|--------|------|
| 23 | 只写正例不写反例 | 反例黑名单成章（本节即示范） |
| 24 | 「必须」措辞代替视觉标记 | 🔴 / 🛑 视觉标记（LLM 扫标记优先于语义） |
| 25 | 两列 fallback（症状/解法） | 三段式（触发条件/一线修复/兜底，见上方「失败处理」） |
| 26 | 过度优化硬凑轮数 | Δ<2 连续 2 轮即停 |
| 27 | runtime 钉死单一平台 | 中立 badge + 中立措辞 |
