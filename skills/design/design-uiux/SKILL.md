...
## 布局决策路径（四媒介 branch 共用的单一真值源，各媒介文件不重复本节）

1. **定媒介 + 理解任务流程**：用户要完成什么 → 决定信息架构。完成判据：媒介已定，任务目标一句话可复述。
2. **三方向初稿门**：出三版差异化结构（标注适合场景 / 优势 / 代价）→ 用户选定。完成判据：三方向已产出且真实分化（见硬门定义），用户已选定其一或已明确要求默认值。
3. **选布局骨架**：按媒介（HTML 栅格 / App 平台导航 / CLI 命令树 / TUI 面板模式）。完成判据：骨架已落到对应 medium 的 layout 文件模板，非泛泛描述。
4. **选组件 + 状态完整性**：见 [ui-ux/interaction-design.md](references/ui-ux/interaction-design.md) §状态完整性（单一真值源）。完成判据：交互组件已对照该节状态模型逐项覆盖，无缺态。
5. **场景自适应**：响应式断点 / 平台主题 / TTY 检测 / resize 重绘。完成判据：目标媒介的自适应分支（若适用）已验证不崩。
6. **自检**：对照各 medium 文件末尾自检清单，浏览器 / 实际环境看实样，禁凭脑补。完成判据：自检清单逐项过一遍，无未勾项遗留未说明。
