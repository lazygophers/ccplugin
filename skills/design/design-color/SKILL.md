---
name: design-color
description: 主题与配色设计——做颜色搭配/调色板/主题/品牌色阶/暗模式的设计决策。触发：选配色/调色/主题/色板/品牌色/暗模式/对比度/色盲/UI风格。按媒介路由 HTML/Web(CSS变量)、原生App(平台token)、CLI(ANSI)、TUI(真彩/256/16降级)。保证可访问性（对比度/色盲安全）。需后端动态系统不适用；UI/UX 布局/组件/交互走姊妹 skill design-uiux。
---
...
## 配色决策路径（四媒介 branch 共用的单一真值源，各媒介 style.md 不重复本节）

1. **定语境**：受众 / 场景 / 情绪（信任→蓝、活力→橙、创新→紫，见 theory.md 语义）。完成判据：受众/场景/情绪三要素均已落笔，非空泛描述。
2. **三方向初稿门**：从风格谱系出三套色板 → 用户选定。完成判据：三方向已产出且真实分化（见硬门定义），用户已选定其一或已明确要求默认值。
3. **配完整色板**：主色（1）+ 辅助（1-2）+ 中性阶（5-7）+ 语义色（4）+ 背景（2-3）。完成判据：五类色槽均已填值，无缺项。
4. **可访问性硬指标**：对比度达标（正文 ≥ 4.5:1 / 大字 ≥ 3:1），颜色传信息处已加图标/文字冗余，暗模式已重调中性阶（降饱和、提对比）。完成判据：对比度/冗余编码/暗模式三项均已核对（见 [color/accessibility.md](references/color/accessibility.md) 检查清单）。
5. **固化 token**：按媒介落地（CSS 变量 / App token / ANSI / 真彩 hex）。完成判据：色值已写成对应媒介的 token 形式，非散落硬编码。
6. **场景自适应**：亮/暗切换、色域降级（TUI）、TTY 关色（CLI）。完成判据：目标媒介的自适应分支（若适用）已验证不崩。
