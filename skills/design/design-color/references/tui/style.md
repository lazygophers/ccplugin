...
**定受众**（power user→vim 风 / 普通→通用风 / 长期→emacs 风）后走顶层 [SKILL.md](../../SKILL.md) §配色决策路径：三方向初稿门（主题+键位组合）→ 并排对比（同界面实机看）→ 固化（色 token + 键位表写死）；本文件只给 TUI 色彩约束 + 现成主题/键位模板。完成判据：受众已定，且已进入顶层三方向初稿门。
