...
CLI 风格 = 命名风格 + 输出样式 + 错误语气 + help 排版的一致性。本文件给方法论 + **现成风格模板**，复制即用。布局 / 场景 / 组件的结构问题在姊妹 skill `design-uiux`，本文件只管色彩与风格落地。

## 选型流程

**定调性**（工具面向开发者 / 运维 / 终端用户？严肃 or 友好？）后走顶层 [SKILL.md](../../SKILL.md) §配色决策路径：三方向初稿门 → 并排对比（同命令三种风格实际输出）→ 固化进 help / 错误 / 输出模板；本文件只给 CLI 命名风格 + 现成模板。完成判据：调性已定，且已进入顶层三方向初稿门。
