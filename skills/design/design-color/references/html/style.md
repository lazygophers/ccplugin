...
风格不是「好看就行」，是「匹配受众、场景、品牌情绪」。本文件给选型方法论 + **现成可用的主题模板**（配色、字号阶、圆角、阴影 token，复制即用）。布局 / 场景 / 组件的结构问题在姊妹 skill `design-uiux`，本文件只管色彩与风格落地。

## 选型流程

**定语境**（受众？场景——营销 / 工具 / 内容 / 企业？情绪关键词？）后走顶层 [SKILL.md](../../SKILL.md) §配色决策路径：三方向初稿门（每方向一套「风格+配色+字体」组合）→ 并排对比（真实初稿上选，不在抽象描述上选）→ 固化为本项目 CSS 变量；本文件只给 HTML 风格谱系 + 现成模板。完成判据：语境已定，且已进入顶层三方向初稿门。
...
色板构成与可访问性硬指标见顶层 [SKILL.md](../../SKILL.md) §配色决策路径 + [color/accessibility.md](../color/accessibility.md)（单一真值源）。
