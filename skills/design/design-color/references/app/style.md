...
App 风格受平台语言（Human Interface Guidelines / Material 3）影响，但仍有品牌表达空间。本文件给选型方法论 + **现成主题模板**（iOS / Android / 桌面各一套，含配色 token）。布局 / 场景 / 组件的结构问题在姊妹 skill `design-uiux`，本文件只管色彩与风格落地。

## 选型流程

**定语境**（受众、品类——工具 / 消费 / 内容 / 企业、情绪）后走顶层 [SKILL.md](../../SKILL.md) §配色决策路径：三方向初稿门 → 并排对比 → 固化 token；本文件只给 App 风格谱系 + 现成模板。完成判据：语境三要素已定，且已进入顶层三方向初稿门。
...
色板构成与可访问性硬指标见顶层 [SKILL.md](../../SKILL.md) §配色决策路径 + [color/accessibility.md](../color/accessibility.md)（单一真值源）。App 专属补充：Dynamic Type / Font Scale 放大后配色对比仍需达标。
