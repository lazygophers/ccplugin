# Semantic Search 优化方案

参考 Serena 项目的设计理念，对 semantic.py 的索引和搜索功能进行全面优化，提高准确率和性能。

## 当前问题分析

### 1. 索引问题
- 📌 **仅支持代码块索引**，不支持符号级别的精细索引
- 📌 **缺乏智能分块**，可能将相关的代码分割
- 📌 **元数据利用不足**，仅存储基本信息
- 📌 **无增量索引机制**，每次都全量重新索引

### 2. 搜索问题
- 🔍 **单一搜索模式**，仅支持纯向量搜索
- 🔍 **缺乏混合搜索**，无法结合关键词匹配（BM25）
- 🔍 **查询理解不足**，不支持查询规范化和扩展
- 🔍 **结果排序简单**，仅按相似度排序

### 3. 准确率问题
- ⚠️ 无法处理多义词和同义词搜索
- ⚠️ 代码的特殊语义未能充分利用
- ⚠️ 缺乏上下文感知的相似度计算
- ⚠️ 函数名称、文件名等重要信息未充分利用

## 优化方案

### 一、索引优化

#### 1.1 增强的分块策略

**目标**：支持多种粒度的代码分块

```python
class SmartCodeChunker:
    """智能代码分块器"""

    def chunk_by_semantic_unit(self, code: str, language: str):
        """语义单元分块
        - 函数/方法（Function）
        - 类定义（Class）
        - 块级注释（Block Comments）
        - 逻辑段落（Logical Blocks）
        """

    def chunk_with_context(self, chunks: List[Dict]):
        """为代码块添加上下文
        - 前置代码（5-10 行）
        - 后置代码（5-10 行）
        - 相关导入和依赖
        """

    def generate_chunk_metadata(self, chunk: Dict):
        """生成增强的元数据
        - 符号名称（函数/类/变量）
        - 符号类型
        - 依赖关系
        - 复杂度指标
        - 代码覆盖范围
        """
```

#### 1.2 符号级索引支持

**目标**：支持精细的符号级搜索

```python
class SymbolIndexer:
    """符号级索引器"""

    def extract_symbols(self, file_path: Path, language: str):
        """提取符号信息
        - 函数/方法定义
        - 类定义
        - 全局变量
        - 常量定义
        - 类型定义（接口、类型别名等）
        """

    def create_symbol_index(self, symbols: List[Dict]):
        """创建符号索引
        - 符号名称索引
        - 符号类型索引
        - 符号文件位置索引
        """

    def index_symbol_relationships(self):
        """建立符号关系
        - 函数调用关系
        - 类继承关系
        - 依赖关系图
        """
```

#### 1.3 多维度元数据存储

**增强元数据字段**：

```python
{
    "id": "chunk_id",
    "file_path": "path/to/file.py",
    "language": "python",

    # 基础信息
    "code": "actual code",
    "code_type": "function",  # function, class, block, etc
    "name": "function_name",
    "start_line": 10,
    "end_line": 25,

    # 增强元数据
    "symbols": ["func_name", "used_var1", "used_var2"],  # 关键符号
    "imports": ["import1", "import2"],  # 相关导入
    "dependencies": ["dep1", "dep2"],  # 依赖项
    "keywords": ["keyword1", "keyword2"],  # 关键词（提取）
    "complexity": 3,  # 代码复杂度
    "has_docstring": True,
    "docstring": "函数说明",
    "is_test": False,

    # 向量信息
    "vector": [...],  # 代码向量
    "doc_vector": [...],  # 文档/注释向量
    "name_vector": [...],  # 名称向量

    "indexed_at": "2025-01-09T...",
}
```

#### 1.4 增量索引机制

**实现文件变更跟踪**：

```python
class IncrementalIndexer:
    """增量索引器"""

    def compute_file_hash(self, file_path: Path) -> str:
        """计算文件 hash"""

    def track_changes(self, root_path: Path) -> Dict[str, str]:
        """跟踪文件变更状态
        - 新增文件
        - 修改文件
        - 删除文件
        """

    def incremental_index(self, changes: Dict[str, str]):
        """增量更新索引
        - 快速跳过未变更文件
        - 只重新索引修改的文件
        - 删除已删除文件的索引
        """
```

### 二、搜索优化

#### 2.1 混合搜索策略

**结合向量搜索和关键词搜索**：

```python
class HybridSearcher:
    """混合搜索器"""

    def vector_search(self, query: str, limit: int = 50):
        """向量语义搜索
        - 生成查询向量
        - 检索语义相似结果
        - 返回前 50 个结果
        """

    def bm25_search(self, query: str, limit: int = 50):
        """BM25 关键词搜索
        - 关键词提取
        - BM25 排序
        - 返回前 50 个结果
        """

    def hybrid_search(self, query: str, limit: int = 10,
                     vector_weight: float = 0.6,
                     keyword_weight: float = 0.4):
        """混合搜索
        - 并行执行向量搜索和 BM25 搜索
        - 按权重组合结果
        - 去重和重排序
        - 返回最终结果
        """

    def rerank_results(self, results: List[Dict],
                      query: str, strategy: str = "fusion"):
        """结果重排序
        策略：
        - RRF（Reciprocal Rank Fusion）
        - Linear Combination
        - Learning-to-Rank
        """
```

#### 2.2 智能查询处理

**优化查询理解**：

```python
class QueryProcessor:
    """查询处理器"""

    def normalize_query(self, query: str) -> str:
        """查询规范化
        - 转换为小写
        - 移除特殊字符
        - 处理空白
        """

    def query_expansion(self, query: str) -> List[str]:
        """查询扩展
        - 同义词替换
        - 缩写扩展（如 func -> function）
        - 相关概念扩展
        """

    def extract_query_intent(self, query: str):
        """提取查询意图
        - 搜索类型判断（定义、实现、调用等）
        - 目标符号类型识别
        - 语言特定语义理解
        """

    def rewrite_query(self, query: str) -> str:
        """查询重写
        - 添加上下文相关的关键词
        - 优化查询语义
        - 转换为多语言
        """
```

#### 2.3 上下文感知搜索

**利用代码结构进行搜索**：

```python
class ContextAwareSearch:
    """上下文感知搜索"""

    def search_with_context(self, query: str,
                           current_file: Optional[str] = None,
                           current_symbol: Optional[str] = None):
        """基于上下文的搜索
        - 提升同一文件中的结果权重
        - 提升相关符号的结果权重
        - 考虑导入关系
        """

    def find_related_symbols(self, symbol_name: str):
        """查找相关符号
        - 调用该符号的代码
        - 被该符号调用的代码
        - 继承/实现关系
        """

    def build_dependency_graph(self):
        """构建依赖图
        - 用于加权搜索
        - 用于上下文计算
        """
```

### 三、准确率改进

#### 3.1 多向量嵌入

**使用多种嵌入方式**：

```python
class MultiVectorEmbedding:
    """多向量嵌入"""

    def encode_code(self, code: str):
        """代码嵌入
        - 使用代码专用模型（BGE Code）
        """

    def encode_documentation(self, doc: str):
        """文档嵌入
        - 使用通用文本模型
        - 处理函数注释和文档
        """

    def encode_name(self, name: str):
        """名称嵌入
        - 强化符号名称的语义
        - 分割驼峰命名
        """

    def hybrid_encode(self, code: str, doc: str, name: str):
        """混合嵌入
        - 结合多个向量
        - 权重组合
        """
```

#### 3.2 代码特定优化

**代码搜索的特殊处理**：

```python
class CodeSpecificOptimization:
    """代码特定优化"""

    def extract_code_semantics(self, code: str, language: str):
        """提取代码语义
        - AST 分析
        - 控制流分析
        - 数据流分析
        """

    def compute_code_similarity(self, code1: str, code2: str):
        """计算代码相似度
        - 结构相似度
        - 功能相似度
        - 实现相似度
        """

    def language_specific_search(self, query: str, language: str):
        """特定语言搜索
        - 考虑语言特有的模式
        - 处理语言特定的语义
        """
```

#### 3.3 结果过滤和验证

**改进结果质量**：

```python
class ResultQualityControl:
    """结果质量控制"""

    def filter_results(self, results: List[Dict], query: str):
        """结果过滤
        - 移除重复结果
        - 移除低相似度结果
        - 移除不相关的测试代码
        """

    def diversity_reranking(self, results: List[Dict]):
        """多样性重排序
        - 保证结果多样性
        - 避免过多相似结果
        """

    def quality_scoring(self, result: Dict, query: str):
        """质量评分
        - 相似度得分
        - 相关性得分
        - 重要性得分（基于使用频率）
        """
```

## 实施路线

### 阶段1：基础优化（✅ 完成）
- ✅ 增强元数据收集
- ✅ 符号提取改进（symbol_extractor.py）
- ✅ 智能查询处理（query_processor.py）

### 阶段2：混合搜索（⏳ 进行中）
- ✅ 实现 BM25 搜索（bm25_searcher.py）
- ✅ 融合向量搜索和关键词搜索（hybrid_searcher.py）
- ✅ 结果重排序（5种融合策略）
- ✅ 集成搜索器（integrated_searcher.py）
- ⏳ 集成到 semantic.py 主程序

### 阶段3：高级特性（待规划）
- 增量索引
- 上下文感知搜索
- 多向量嵌入

## 性能指标

### 搜索准确率
| 指标 | 当前 | 目标 | 改进 |
|------|------|------|------|
| Top-5 相关性 | 60% | 85% | +25% |
| Top-10 相关性 | 70% | 90% | +20% |
| 平均排名位置 | 4.5 | 2.0 | 改善55% |

### 搜索性能
| 指标 | 当前 | 目标 | 改进 |
|------|------|------|------|
| 平均查询时间 | 200ms | <100ms | 50% 加速 |
| 索引大小 | 基准 | +30% | 为元数据 |
| 索引时间 | 基准 | 相似 | 增量索引 |

## 关键改进点

1. **符号级精度** - 从块级转向符号级索引
2. **混合搜索** - 结合向量和关键词搜索
3. **上下文感知** - 利用代码结构
4. **准确性验证** - 多维度的相似度计算
5. **性能优化** - 增量索引和缓存

## 参考资源

- **Serena**: https://github.com/oraios/serena
  - 符号级别的代码检索
  - LSP 集成
  - 精准的代码理解

- **BM25**: 经典关键词搜索算法
- **RRF**: 混合排序融合算法
