...
def collect_best_paths(root: Path, wanted_names: set[str], *, max_depth: int = 4, max_files: int = 2500) -> dict[
	str, Path]:
...
def render_statusline(payload: dict) -> str:
	...
	thinking_on = str(thinking_enabled_raw).strip().lower() in {"true", "1", "yes",
	                                                            "on"} if thinking_enabled_raw is not None else False
	...
	if not needs_third_line:
		...
		return f"{line1}\n{line2}".rstrip()
	if rows < 3:
		folded = fit_segments([line2] + meta_parts + (
			[style(f"v{version.lstrip('vV')}", fg=CATPPUCCIN["subtle"], dim=True)] if version and "v" not in strip_ansi(
				"".join(meta_parts)) else []), sep=sep_dot, max_width=cols)
		return f"{line1}\n{folded}".rstrip()
	return f"{line1}\n{line2}\n{line3}".rstrip()
