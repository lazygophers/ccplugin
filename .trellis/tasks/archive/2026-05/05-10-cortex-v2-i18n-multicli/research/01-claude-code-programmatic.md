...
| Flag                                                                       | 作用                                                                                                                                                                                             |
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ★ `--settings <file-or-json>`                                              | 加载额外 settings (env/perm/hook) — 即 ccplugin 项目惯用 `~/.claude/settings.glm-4.7-flash.json` 切廉价 model                                                                                    |
...
| **隔离**   | `--bare` + `--setting-sources user` + 显式 `--settings ~/.claude/settings.glm-4.7-flash.json`        |
...
timeout 600 claude \
  ...
  --settings "$HOME/.claude/settings.glm-4.7-flash.json" \
...
ccplugin 项目惯例: 优化 commands/skills/agents 后用 `--settings ~/.claude/settings.glm-4.7-flash.json -p` 廉价模型回归测试 (CLAUDE.md §代码质量检查规范)。
...
9. **复用 ccplugin 现有 `--settings` 模式** — 把 cortex 维护脚本统一用 `~/.claude/settings.glm-4.7-flash.json`, 与项目其他校验脚本一致; 团队/CI 可换 `cch-*` 系列。
