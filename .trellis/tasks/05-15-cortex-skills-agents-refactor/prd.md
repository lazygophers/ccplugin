...
7. **回归验证**: 跑 `claude --settings ~/.claude/settings.glm-4.7-flash.json -p "<待测>"` 对每个保留 skill 的 SKILL.md 入口验 AI 识别正确
...
- 回归命令: `claude --settings ~/.claude/settings.glm-4.7-flash.json -p "<待测>" --output-format stream-json | jq -r 'select(.type == "result" and .subtype == "success") | .result'`
