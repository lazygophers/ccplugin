...
CCPlugin Market 是 Claude Code 的插件生态，提供 7+ 沿插件（Memory、Cortex、Git、Notify、Version、Task、Llms），同时内置任务编排系统（Trellis）和智能体配置框架。
...
| 层 | 职责 | 关键文件 |
|----|------|----------|
| **插件市场层** | Claude Code 扩展能力集合 | `plugins/tools/cortex/`, `plugins/memory/`, `plugins/tools/notify/` |
...
| 插件 | 入口 | 职责 |
|------|------|------|
| **cortex** | `plugins/tools/cortex/scripts/cli/` | 知识摄取、搜索、摘要、图谱管理 |
...
plugins/memory/scripts/hooks.py
...
```
plugins/tools/cortex/scripts/cli/deep_search.py
plugins/tools/cortex/scripts/cli/ingest_file.py
plugins/tools/cortex/scripts/cli/ingest_url.py
plugins/tools/cortex/scripts/cli/save.py
plugins/tools/cortex/scripts/cli/search.py
plugins/tools/cortex/scripts/cli/memory.py
plugins/tools/cortex/scripts/cli/lib/evolution.py
plugins/tools/cortex/scripts/cli/lib/extractors/html.py
plugins/tools/cortex/scripts/cli/lib/remote.py
```
Cortex 是最复杂的插件，CLI 子命令众多，lib/ 下有独立的提取器和远端同步逻辑。
